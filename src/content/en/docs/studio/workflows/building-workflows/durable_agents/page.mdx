...
- **Telemetry and observability**: the plugin uses [`get_mistral_client()`](/studio/workflows/building-workflows/mistral_client) under the hood, so all LLM calls include telemetry and workflow tracing metadata by default
...
Use a mapping instead of raw values because activity inputs are persisted to the workflow's [event history](/studio/workflows/getting-started/core_concepts/events). Loading secrets from the worker's environment and passing them directly would expose them there.
...
A Streamable HTTP server usually needs credentials on every request: a bearer token gating the endpoint and/or a per-request integration token. Set them from the worker's environment so no secret is serialized into the workflow's [event history](/studio/workflows/getting-started/core_concepts/events).
...
Agent activity emits the same workflow and activity events as any other workflow. Subscribe to them live via [Streaming](/studio/workflows/building-workflows/streaming) and [Consuming Streaming Events](/studio/workflows/building-workflows/consuming_events), or replay the full event history after the fact.
