...
Les juges sont des évaluateurs basés sur des modèles de langage que vous pouvez utiliser dans le [SDK Evaluation](/studio/observability/evaluations#installation). Au lieu d'une fonction basée sur des règles, un juge appelle un modèle pour noter chaque sortie. Ils sont utiles pour des critères difficiles à capturer avec du code, comme l'utilité, l'exactitude factuelle ou le ton.

:::note
Assurez-vous que le package `mistralai-evaluations` est installé avant de suivre les exemples de cette page. Consultez [Évaluations hors ligne → Installation](/studio/observability/evaluations#installation).
:::
