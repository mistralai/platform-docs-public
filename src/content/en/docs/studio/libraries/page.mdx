...
Document Library is a built-in [agent tool](/studio/agents/agent-tools#built-in-tools) that lets agents search through your Libraries. To use it, create an agent with the `document_library` tool and pass the `library_ids` you want it to access.
...
:::tip
The [web search](/studio/agents/agent-tools/websearch) tool also uses references. For more on citations, see the [citations guide](/studio/conversations/citations).
:::

## Going further

- [RAG quickstart](/studio/knowledge-rag/rag_quickstart): build a retrieval-augmented generation pipeline from scratch.
- [Embeddings](/studio/knowledge-rag/embeddings): generate vector embeddings for custom RAG pipelines.
- [Agent tools](/studio/agents/agent-tools): explore other built-in tools like web search, Code Interpreter, and image generation.
