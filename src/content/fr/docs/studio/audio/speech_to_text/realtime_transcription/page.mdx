...
La transcription en temps réel s’exécute via un WebSocket :

```text
wss://api.mistral.ai/v1/audio/transcriptions/realtime?model=voxtral-mini-transcribe-realtime-2602
```

Le paramètre de requête `model` est obligatoire. Les SDK ci-dessous ouvrent cette connexion pour vous. Pour les clients navigateur, voir [Authentification client](/studio/audio/speech_to_text/realtime_transcription/client_auth).
