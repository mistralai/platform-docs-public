...
Avec `DOCUMENT_PER_CHUNK`, `create_schema()` injecte automatiquement les champs standard des fragments (contenu, embedding, identité, métadonnées). Consultez le [Modèle de document](/studio/search/search-toolkit/document-model) pour savoir comment les fragments sont identifiés et [Gérer le schéma](migrations) pour définir le mode.
