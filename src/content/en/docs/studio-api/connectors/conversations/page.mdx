...
After a Connector is [registered](/studio-api/connectors/management) and [authenticated](/studio-api/connectors/management#get-auth-url) (if required), you can attach it to any conversation.
...
:::tip
You can require user approval before a tool runs by adding `requires_confirmation` to the `tool_configuration`. This is useful for sensitive actions like sending emails or modifying data. See [Human-in-the-loop](/studio-api/connectors/confirmation) for the full confirmation flow.
:::
