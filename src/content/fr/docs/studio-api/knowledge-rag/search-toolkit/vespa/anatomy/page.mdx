...
Cette page explique les concepts qui constituent une application Vespa : le package d'application, les schémas, les champs, les profils de classement et les profils de requête. Pour savoir comment les créer et les configurer, voir [Gérer le schéma](migrations).
...
Vous ne rédigez jamais ces fichiers manuellement. Ils sont générés en mémoire à partir de vos [migrations](migrations) et téléchargés sur le cluster Vespa. Vous pouvez les inspecter avec `mistral-vespa generate` (voir [référence du CLI](cli)).
...
- **[Gérer le schéma](migrations)** — Comment créer des schémas, des champs et un classement via les migrations
