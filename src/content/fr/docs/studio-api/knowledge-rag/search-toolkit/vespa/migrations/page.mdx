...
Gérez les schémas de votre application Vespa via des migrations Python. Pour comprendre les concepts liés aux schémas, aux champs et au classement, consultez la page [Anatomie d’une application Vespa](anatomy).
...
`mistral-vespa migrate` détecte les migrations, les exécute dans l’ordre, construit le package de l’application et le **télécharge**. Consultez les guides [Développement local](local-development) et [Déploiement et exploitation](operations) pour plus de détails.
...
- **[Développement local](local-development)** — Cycle complet de développement local
- **[Référence CLI](cli)** — Ensemble complet des options de la CLI
