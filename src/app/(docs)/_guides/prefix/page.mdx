...
Prefixes are a feature that can be impactful for many use cases and scenarios. While the concept is simple, the possibilities are broad.

We'll now dig into a few different examples and explore the hidden potential of prefixes!
...
For all of the following examples, we'll need to set up our client.
...
For that, we'll define a `system` prompt!
...
Optionally, you can remove the prefix if you don't expect it to be part
of the answer.
...
In this segment, we'll explore how we can make use of different
...
For this, we'll set a prefix in the same way we would start a
dialogue.
...
We're getting there! Now let's have a full conversation with a
...
The idea we're going to explore here is similar to the "Language
...
very fixed spectrum. For this, we'll use our own safe prompt:
