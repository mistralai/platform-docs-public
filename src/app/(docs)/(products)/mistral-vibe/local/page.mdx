...
Once the server is running locally on port **8080**, you can quickly switch from the Devstral API to your local server by using `/config` in Mistral Vibe and changing the model to **"local"**. By default, Mistral Vibe uses port **8080** as the local server.  
For more information on configuring Mistral Vibe, please refer to the [configuration page](/mistral-vibe/terminal/configuration).
:::
...
```
Here, `--port` specifies the port for the server. We recommend using **8080**, as it is the default port for Mistral Vibe. Otherwise, you will need to configure Mistral Vibe to use the correct port.
...
Once the server is up and running, you can use Mistral Vibe to interact with your local model by typing `/config` and selecting the **"local"** model.
...
To learn how to create a preset or change providers and models, please refer to the [Change Providers and Models](/mistral-vibe/terminal/configuration#change-providers) configuration section.
