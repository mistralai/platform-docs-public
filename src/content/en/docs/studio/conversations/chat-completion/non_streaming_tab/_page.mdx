...
Note that the response content of the model can have **interleaved events** instead of a single string, such as [citations](/studio/conversations/citations) and [tool calls](/studio/conversations/function-calling).  
