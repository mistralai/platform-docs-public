...
Lors du déploiement de LLM en production, différents secteurs peuvent nécessiter différents niveaux de guardrailing. Par exemple, dans une application de chatbot, il peut être essentiel de garantir que le contenu généré soit sûr et respectueux.
Dans d'autres applications, il peut être nécessaire de détecter et de filtrer le contenu nuisible ou les informations personnelles identifiables (PII).

Nous proposons deux méthodes pour protéger vos applications :
- **[Guardrails personnalisés](#custom-guardrails)** *(recommandé)* : déclarez des règles de modération directement dans vos requêtes API. Pas d’appels séparés ni de logique de seuil à implémenter dans votre code. Prend en charge les Chat completion, les Conversations et la configuration au niveau agent.
- **[API de modération](#moderation)** : une API dédiée pour classifier le texte selon les catégories de politique, pour les pipelines personnalisés où vous avez besoin de scores bruts et d'un contrôle total.
...
:::note
`mistral-moderation-2411` a été déprécié le 31 mars 2026. Voir [Mistral Moderation 2411](/resources/deprecated/guardrailing/mistral_moderation_2411) pour la référence héritée.
:::
...
<ExplorerTabs id="endpoints">
  <ExplorerTab value="raw-text-endpoint" label="Point de terminaison pour texte brut">
    <RawTextEndpointTab/>
  </ExplorerTab>
...
:::note[ ]
Le seuil de politique est déterminé sur la base des performances optimales de notre ensemble de test interne.
Vous pouvez utiliser le score brut ou ajuster le seuil selon vos cas d'usage spécifiques.

Nous avons l'intention d'améliorer continuellement le modèle sous-jacent du point de terminaison de modération.
Les politiques personnalisées qui dépendent des `category_scores` peuvent nécessiter un réétalonnage.
:::
...
Notre [cookbook de modération](https://colab.research.google.com/github/mistralai/cookbook/blob/main/mistral/moderation/system-level-guardrails.ipynb) fournit un exemple concret d'utilisation du service de modération pour mettre en œuvre des garde-fous au niveau système.  
Pour une vue plus large, il existe également un [cookbook plus exploratoire](https://colab.research.google.com/github/mistralai/cookbook/blob/main/mistral/moderation/moderation-explored.ipynb).

<SectionTab as="h2" variant="secondary" sectionId="faq">FAQ</SectionTab>

<Faq>
  <FaqItem question="Quelle est la distribution des résultats faux positifs et faux négatifs sur les nouveaux modèles d'API de modération ? Plus précisément, seront-ils plus susceptibles de signaler quelque chose comme nuisible alors que ce n'est pas le cas, ou de ne pas signaler quelque chose qui est nuisible ?">
    Sur notre jeu de tests interne, les politiques ont une précision comprise entre [0,8 – 1,0] et un rappel entre [0,7 – 1,0]. Nous vous recommandons d’utiliser les scores bruts plutôt que les réponses booléennes si vous avez des objectifs d’application spécifiques (par exemple, réduire les faux positifs) et d’ajuster les seuils en conséquence. Nous recueillons en continu le retour des utilisateurs sur les performances et améliorons nos modèles.
  </FaqItem>
</Faq>
...
<SectionTab as="h2" variant="secondary" sectionId="conversations-guardrails">Guardrails pour Conversations</SectionTab>
