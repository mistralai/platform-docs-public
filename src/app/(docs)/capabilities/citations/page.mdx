...
Citations enable models to ground their responses and provide references, making them a useful feature for Retrieval-Augmented Generation (RAG) and agentic applications. This feature allows the model to provide the source of the information extracted from a document or chunk of data from a tool call.
...
Below is an example of references from Wikipedia using tool calls.

<SectionTab as="h2" variant="secondary" sectionId="setup">Set up tool</SectionTab>
...
In this case, we'll create a `get_information` tool that returns the references mentioned previously.
...
To get started, initialize the client with your API key as usual.
...
The model wants to call the `get_information` tool to retrieve information from the external source.
...
Finally, you can make the final chat request to the model with the updated chat history, providing the **original query**, the model's **tool call**, and the **tool result**.
...
You can find a comprehensive cookbook exploring citations and references using RAG with Wikipedia [here](https://colab.research.google.com/github/mistralai/cookbook/blob/main/mistral/rag/mistral-reference-rag.ipynb).
This template will help you get started with web search and document grounding with citations.