...
You set up your environment and hardware in the **cerebrium.toml** file that was created using the init function above. Here we are using the CUDA 12 base image, a Ampere A10 GPU etc.
...
We need to add our Hugging Face token to our Cerebrium Secrets since using the Mistral model requires authentication. On first deploy, it will download the model and store it on disk therefore for subsequent calls it will load the model from disk.
