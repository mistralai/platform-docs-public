...
Les étapes facultatives de `DocumentProcessor` (comme un `ChunkEnricher` ou un `SummaryEnricher`) ajoutent des métadonnées à chaque fragment avant l’indexation, par exemple un résumé généré par un LLM ou des tags d’entités.
...
- **[Chargeurs de fichiers](/studio/search/search-toolkit/ingestion/loaders)** : chargement de fichiers depuis un système de fichiers, un stockage cloud ou des sources personnalisées
- **[Extracteurs de documents](/studio/search/search-toolkit/ingestion/extractors)** : extraction de contenu à partir de PDF, HTML, tableurs, etc.
- **[Sépareurs de texte](/studio/search/search-toolkit/ingestion/splitters)** : division des documents en fragments avec une taille et un chevauchement optimaux
- **[Enrichisseurs de fragments](/studio/search/search-toolkit/ingestion/enrichers)** : ajout de métadonnées personnalisées pour le filtrage et le classement
- **[Embedders](/studio/search/search-toolkit/ingestion/embedders)** : conversion de texte en vecteurs pour la recherche sémantique
- **[Index de recherche](/studio/search/search-toolkit/search-index)** : stockage et recherche dans les fragments indexés
