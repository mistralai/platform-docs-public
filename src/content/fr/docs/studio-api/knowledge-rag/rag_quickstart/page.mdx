...
La génération augmentée par récupération (RAG) combine la récupération avec la génération de modèles. Elle vous aide à répondre à des questions ou à générer du contenu à partir de connaissances externes. Il existe deux étapes principales dans le RAG :
...
2. **Génération** : insérez les informations pertinentes dans le prompt pour que le modèle puisse générer une réponse ancrée dans ce contexte.

Utilisez ce guide quand vous souhaitez comprendre et construire un pipeline RAG à partir de zéro. Si vous recherchez une fonctionnalité gérée qui ingère, vectorise etue recherche des documents pour vous, utilisez [Bibliothèques](/studio-api/libraries). Si vous voulez ancrer les réponses dans des sources connectées telles que Google Drive ou SharePoint, utilisez [Connecteurs](/studio-api/connectors).

Vous trouverez des guides RAG plus détaillés dans la section Exemples RAG ci-dessous.

<SectionTab as="h1" sectionId="rag-from-scratch">RAG from scratch</SectionTab>

<SectionTab as="h2" variant="secondary" sectionId="build-basic-rag">Construire un pipeline RAG basique</SectionTab>
...
Dans un système RAG, le document est divisé en fragments plus petits afin que la récupération puisse identifier les informations les plus pertinentes ultérieurement. Dans cet exemple, nous divisons le texte par caractère, combinons 2048 caractères dans chaque fragment et obtenons 37 fragments.
...
Pour chaque fragment de texte, nous devons ensuite créer des embeddings, qui sont des représentations numériques du texte dans l'espace vectoriel. Les mots ayant des significations similaires doivent se situer à proximité ou avoir une distance plus courte dans l'espace vectoriel.
Pour créer un embedding, utilisez l'endpoint API embedding et le modèle d'embedding `mistral-embed`. Nous créons `get_text_embedding` pour obtenir l'embedding d'un fragment de texte, puis utilisons une compréhension de liste pour obtenir les embeddings de texte pour tous les fragments.
...
- **RAG avec LlamaIndex** : visitez notre [recueil de recettes RAG agentiques avec LlamaIndex](/resources/cookbooks/third_party-llamaindex-llamaindex_agentic_rag) pour apprendre comment utiliser LlamaIndex avec l'API Mistral afin d'exécuter des requêtes complexes sur plusieurs documents à l'aide d'un agent ReAct.
- **RAG avec Haystack** : visitez notre [recueil de recettes de chat avec documents avec Haystack](/resources/cookbooks/third_party-haystack-haystack_chat_with_docs) pour découvrir comment utiliser Haystack avec l'API Mistral pour le chat avec des documents.
