...
Below, we have an overview on how to use the Chat Completion API to generate text and engage in conversational interactions with our models.
...
Chat Completions can be used for multi-turn conversations. This means you can send multiple messages back and forth between the user and the assistant. This is useful for applications like chatbots, where the user can have a conversation with the assistant.

It's worth noting that you may have different events interleaved between these interactions, such as tool calls for function calling, or even handoffs when handling agents.
...
- A `stop` sequence allows forcing the model to stop generating after one or more chosen tokens or strings. The output won't contain the stop sequence.
...
This was a simple introduction to our Chat Completions service, but we have a lot more to offer. We recommend taking a look at our [Vision](../vision) capabilities, [Function Calling](./function-calling), [Predicted Outputs](./predicted-outputs), [Structured Outputs](./structured-output), and much more.
