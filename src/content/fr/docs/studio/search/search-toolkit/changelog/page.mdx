...
Search Toolkit utilise désormais un modèle de document unifié construit autour de `Document` et `DocumentChunk`, avec une identité déterministe dérivée d’un `source_id` et d’un `locator`. Consultez la page [Modèle de document](/studio/search-toolkit/document-model) pour plus de détails.
