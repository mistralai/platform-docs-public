...
Predicted Outputs **optimizes response time** by using known or predictable content.
...
**Predicted Outputs work best in scenarios where you need to regenerate text documents or code files with minor modifications.** The key parameter is `prediction`, which lets you define predicted outputs. For example, imagine you want your model to update the model used in a fine-tuning job. You can include the code snippet you'd like to modify as both the user prompt and the predicted output.
...
<Faq>
  ...
  <FaqItem question="How do predicted outputs affect pricing?">
    Currently, predicted outputs don't impact pricing.
  </FaqItem>
  <FaqItem question="Which parameters aren't supported when using predicted outputs?">
    `n` (number of completions to return for each request) isn't supported when using predicted outputs.
  </FaqItem>
  <FaqItem question="Does the position of certain sentences or words in the prediction matter?">
    No, the placement of sentences or words in your prediction doesn't affect its effectiveness. Predictions can appear anywhere within the generated response and still help reduce the API's output latency.
  </FaqItem>
</Faq>