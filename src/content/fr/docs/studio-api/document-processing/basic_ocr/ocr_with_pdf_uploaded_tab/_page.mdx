...
Vous pouvez également télécharger un fichier PDF dans notre cloud et obtenir les résultats OCR du PDF téléchargé en récupérant une URL signée.

<SectionTab as="h3" variant="secondary" sectionId="upload-a-file">Télécharger un fichier</SectionTab>

Tout d'abord, vous devrez télécharger votre fichier PDF vers notre cloud. Ce fichier sera stocké et accessible uniquement via une clé API.
...
Une fois le fichier téléchargé, vous pouvez le récupérer à tout moment.
...
Pour les tâches OCR, vous pouvez obtenir une URL signée pour accéder au fichier. Un paramètre optionnel `expiry` permet d'expirer automatiquement l'URL signée après n heures.
...
<SectionTab as="h3" variant="secondary" sectionId="get-ocr-results">Obtenir les résultats OCR</SectionTab>

Vous pouvez maintenant interroger le endpoint OCR avec l'URL signée.
...
Une fois l'OCR terminé, vous pouvez éventuellement supprimer le fichier PDF de notre cloud, sauf si vous souhaitez le réutiliser ultérieurement.
