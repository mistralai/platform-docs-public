...
Chaque fragment est un événement `speech.audio.delta` contenant un fragment audio encodé en base64. Le flux se termine par un événement `speech.audio.done` contenant les informations d'utilisation.

:::tip
Utilisez `response_format="pcm"` pour obtenir la latence de streaming la plus faible (~0,7 s avant le premier audio contre ~2 s pour `mp3`).
:::
