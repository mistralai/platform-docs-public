...
For the canonical list of workflow and activity event types, see [Core Concepts > Events > Event types](/studio/workflows/getting-started/core_concepts/events#event-types). The streaming API additionally surfaces **`CUSTOM_TASK_*`** events emitted from inside activities (started, in-progress, completed, failed, timed out, canceled). Use these to publish progress updates from long-running activities; see [Streaming](/studio/workflows/building-workflows/streaming) for how to publish them.
