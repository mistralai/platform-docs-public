...
Le contenu de confiance comprend l'un des éléments suivants dans le répertoire ou ses ancêtres :

- Un répertoire `.vibe/` (configuration, agents, compétences, prompts, hooks et outils)
- Un répertoire `.agents/` (compétences standard Agent Skills)
- Un fichier `AGENTS.md` (instructions d'agent au niveau du projet)

Fonctionnement :

- Lorsqu’une session interactive démarre dans un répertoire contenant du contenu de confiance et que l’état de confiance est inconnu, Vibe demande si le dossier peut être approuvé.
- La boîte de dialogue de confiance liste les fichiers de confiance détectés et signale les risques (comme les serveurs MCP ou les permissions d'outils accordant un accès étendu). Elle propose la racine du dépôt git comme cible de confiance lorsque le répertoire se trouve dans un dépôt git
...
Vibe Code Web exécute des sessions dans une sandbox gérée associée à un dépôt GitHub. Les approbations s'affichent dans l'interface utilisateur du navigateur, et les actions au niveau du dépôt (création de branches, poussée de commits, ouverture de pull requests) suivent les autorisations GitHub que vous avez accordées lors de la connexion du dépôt.
