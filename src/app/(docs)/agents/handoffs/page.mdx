...
When creating and using agents, often with access to specific tools, there are moments where it's useful to call other agents mid-action. This ability to give agents tasks or hand over a conversation to other agents is called **Handoffs**.
...
There's no limit to how many chained handoffs a workflow can have. You're free to create multiple agents using diverse tools, models, and handoffs, and orchestrate your own workflow using these agents.
...
First, let's create several agents with different tasks and capabilities.
...
<SectionTab as="h2" variant="secondary" sectionId="define-handoffs-responsibilities">Define handoff responsibilities</SectionTab>
...
Once all our agents are created, we update our previously defined agents with a list of available `handoffs`.
<Tabs groupId="code">
<TabItem value="python" label="python" default>
...
It's now possible to have a chain of actions by sending a request to the `finance_agent`.

We also provide the parameter `handoff_execution`, which currently has two modes: `server` or `client`.

- `server`: Runs the handoff internally on our cloud servers. This is the default setting.
- `client`: When a handoff is triggered, a response is returned directly to the user, letting them handle the handoff with full control.
