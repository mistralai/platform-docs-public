...
You can find more information [here](/studio-api/agents/agent-tools/websearch).
  </ExplorerTab>
...
If the agent has guardrails configured, they are automatically applied to all conversations. You can override them by passing a `guardrails` field directly on `POST /v1/conversations` requests with a model. See [Custom guardrails](/studio-api/safety-moderation#custom-guardrails) for details.
...
For more information regarding handoffs visit [this section](/studio-api/agents/handoffs).
