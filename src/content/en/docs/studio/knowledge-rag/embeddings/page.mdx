...
If you want a managed feature that ingests, vectorizes, and searches documents for you, use [Libraries](/studio/search/libraries). If you want to search connected sources such as Google Drive or SharePoint, use [Connectors](/studio/connectors).

<SectionTab as="h1" sectionId="services">Services</SectionTab>

We provide two embedding models:

- [Text embeddings](/studio/knowledge-rag/embeddings/text_embeddings): embed a wide variety of text with a general-purpose embedding model.
- [Code embeddings](/studio/knowledge-rag/embeddings/code_embeddings): embed code databases and repositories for code retrieval.
...
For a quick example and introduction on how to use embeddings for RAG, see [RAG Quickstart](/studio/knowledge-rag/rag_quickstart).
