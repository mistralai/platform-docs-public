...
Below is an example of how to use `bbox_annotation_format` and `document_annotation_format` together to extract information from a document.
...
You can also provide a description for each entry. The description is used as detailed information and instructions during the annotation. The following example includes a description for the BBox annotation but not for the document annotation:
...
After defining your document annotation schema, you may need to provide more context and instructions about the annotation and document. You can pass a `document_annotation_prompt` that serves as a high-level system prompt for the annotation step, providing further context and instructions on how the annotation should be done. An example prompt:
...
In production, you may want to provide context about your use case and refine instructions further if the output doesn’t match expectations exactly.
...
The BBox and document annotation features let you extract data and annotate images from the original document as well as the full document. Below is one of the images extracted by our OCR processor.
...
The extracted image is provided in base64-encoded format.
...
You can annotate the image with the model schema you want. Below is an example output.
...
The document annotation provides the full document annotation. Below is an example output.
