...
# Vertex AI
Our open and commercial models can be deployed on the Google Cloud Vertex AI platform as fully managed endpoints. Mistral models on Vertex AI are serverless services, so you don't have to manage any infrastructure.
...
Available models expose a REST API that you can query using our SDKs or plain HTTP calls.

To run the examples below:
- Install the `gcloud` CLI to authenticate against the Google Cloud APIs. Refer to the [Google Cloud authentication documentation](https://cloud.google.com/docs/authentication/provide-credentials-adc#google-idp) for more details.
