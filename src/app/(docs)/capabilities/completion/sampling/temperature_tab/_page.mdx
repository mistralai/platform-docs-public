...
To better understand the underlying principle and impact it has on the probability distribution, here is a visualization of the Temperature with a simple prompt:
...
The model answered mostly with Dragon! Let's try with a higher temperature to get more diverse outputs, setting it to `temperature = 1`.
...
There's no one-size-fits-all temperature for all use cases, but some guidelines can help you find the best value for your applications.
