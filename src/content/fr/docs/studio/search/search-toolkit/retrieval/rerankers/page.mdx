...
| `top_k` | int \| None | `None` | Nombre de résultats à renvoyer après le reranking (`None` renvoie tout) |
...
Utilise un modèle cross-encoder hébergé et optimisé pour le reclassement. Cette méthode est plus rapide que le reclassement par modèle de langage, mais nécessite un serveur de modèle dédié.
...
| `top_k` | int \| None | `None` | Nombre de résultats à renvoyer (`None` renvoie tout) |
| `timeout` | float | 300.0 | Délai d’expiration de la requête en secondes |
...
- Fusion qui ne nécessite pas de normalisation des scores
