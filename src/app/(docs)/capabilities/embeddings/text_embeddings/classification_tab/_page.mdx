...
After we've trained the classifier with our embeddings data, we can try to classify other text:
...
Additionally, let's look at the performance using fastText embeddings in this classification task. The classification model achieves better performance with our embeddings model compared to fastText embeddings.
...
Our method using our embeddings model achieves a precision of **98.75%**, while the fastText embeddings method achieves a precision of 86.25%.