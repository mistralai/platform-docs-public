...
- A `tool message` only appears in the context of **function calling**, it is used at the final response formulation
  ...
  the [guide](/studio/agents/agent-tools/function-calling).
...
- The `safe_prompt` flag is used to force chat completion to be moderated against sensitive content (see [Guardrailing](/studio/safety-moderation)).
...
This was a simple introduction to our chat completions service, however we have a lot more to offer we recommend taking a look; from [Vision](/studio/conversations/vision) capabilities, to [Function Calling](/studio/agents/agent-tools/function-calling), [Predicted Outputs](/studio/conversations/advanced/predicted-outputs), [Structured Outputs](/studio/conversations/structured-output) and much more.
