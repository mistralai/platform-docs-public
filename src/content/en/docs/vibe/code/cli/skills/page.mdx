...
2. **Project-level** skills in `./.vibe/skills/` or `./.agents/skills/`, checked in the working directory (when [trusted](/vibe/code/safety-approvals-permissions#trusted-folders)) and in any directory added with `--add-dir`.
3. **User-level** skills in `~/.vibe/skills/` or `~/.agents/skills/`.
