...
:::note
**Python Version**: Before running the following script, make sure you have installed the `portaudio` package via the `brew install` command, as well as the `mistralai[realtime]`, `pyaudio` and `rich` packages via the `pip install` command.
:::
