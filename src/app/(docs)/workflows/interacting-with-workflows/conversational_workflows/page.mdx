...
| `FileWithMetadataField` | File upload with metadata | `description`, `multiple` (default `False`)                                        |
...
### FileWithMetadataField

Unlike `FileField` which only provides signed URLs, `FileWithMetadataField` provides structured metadata including filename, URL, and content type. Use `FileWithMetadataValue` as the type annotation.

<Tabs>
  <TabItem value="python" label="Python">

```python
from mistralai.workflows.conversational import FileWithMetadataField, FileWithMetadataValue

# Single file upload with metadata
document: FileWithMetadataValue = FileWithMetadataField(description="Upload a document")

# Multiple file uploads with metadata
attachments: list[FileWithMetadataValue] = FileWithMetadataField(description="Upload files", multiple=True)
```
...
Each `FileWithMetadataValue` has the following fields:
