...
Le prédicat de condition est généralement activé par un [signal](/studio/workflows/interacting-with-workflows/signals) envoyé depuis l’extérieur du workflow (appel API, autre service ou action utilisateur). Le workflow reste en pause sans consommer de ressources de calcul jusqu’à ce que le prédicat soit à `True` ou que le délai soit dépassé.

:::tip
**Vous créez une interaction de type chat ?** Utilisez plutôt [`wait_for_input()`](/studio/workflows/interacting-with-workflows/conversational_workflows) issu de `InteractiveWorkflow`. Ce primitive intégré permet de solliciter l’utilisateur et d’attendre une réponse — sans définition de signal ni prédicat personnalisé à écrire, avec prise en charge native des formulaires structurés, confirmations et délais.
:::
...
Si votre cas repose fortement sur l’intervention humaine, consultez aussi [Signals](/studio/workflows/interacting-with-workflows/signals) pour définir et émettre le déclencheur.
