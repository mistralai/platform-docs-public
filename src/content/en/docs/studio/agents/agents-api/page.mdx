...
If the agent has guardrails configured, they are automatically applied to all conversations. You can override them by passing a `guardrails` field directly on `POST /v1/conversations` requests with a model. See [Custom guardrails](/studio/safety-moderation#custom-guardrails) for details.
...
For more information regarding handoffs visit [this section](/studio/agents/handoffs).
