...
:::info
Encryption can be combined with [payload offloading](/studio/workflows/building-workflows/payload_offloading) and [compression](/studio/workflows/building-workflows/compression). Payloads are compressed and offloaded before encryption runs last, so the orchestrator only ever sees an encrypted reference.
:::
