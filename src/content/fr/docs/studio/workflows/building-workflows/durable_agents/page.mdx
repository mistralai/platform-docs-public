...
Un agent durable est un agent LLM dont la boucle (appels au modèle, utilisation d'outils, transmissions) s'exécute **au sein d'un workflow**, ce qui permet de préserver son état lors de crashs ou de redémarrages. Le plugin Mistral (`mistralai-workflows[mistralai]`) fournit les briques fondamentales pour construire ce type d'agent : `Agent`, `Runner`, `RemoteSession` et `LocalSession` (expérimental).
...
- **Télémétrie et observabilité** : le plugin utilise [`get_mistral_client()`](/studio/workflows/building-workflows/mistral_client) en interne, de sorte que tous les appels LLM incluent la télémétrie et les métadonnées de traçage des workflows par défaut
...
**Transmettre des identifiants à un serveur stdio**

Certains serveurs MCP stdio nécessitent des identifiants (par exemple un token de bot) pour s'authentifier. Utilisez `env_mapping` pour les injecter dans le sous-processus depuis l'environnement du worker. Chaque entrée du mapping associe le nom de la variable du sous-processus à celui de la variable du worker :
...
Pourquoi passer un mapping plutôt que les valeurs brutes ? Les paramètres d'entrée des activités sont conservés dans l'[historique des événements](/studio/workflows/getting-started/core_concepts/events) du workflow ; charger les secrets depuis l'environnement du worker et les transmettre directement les y exposerait donc. Avec `env_mapping`, seuls les noms des variables sont sérialisés ; les valeurs secrètes sont lues dans l'environnement du worker au sein de l'activité et ne le quittent jamais. Passer un mapping permet aussi d'exécuter plusieurs instances du même serveur avec des identifiants différents sur un seul worker. Si une variable de worker déclarée est absente de l'environnement, l'activité lève une erreur.
...
<SectionTab as="h2" variant="secondary" sectionId="streamable-http-mcp-server">Serveur MCP Streamable HTTP</SectionTab>

Pour les serveurs MCP distants qui utilisent le transport Streamable HTTP (`--transport http`), tels qu'un serveur auto-hébergé derrière un point de terminaison HTTP :

:::note
Nécessite `mistralai>=2.8.0` (la version qui fournit le client Streamable HTTP). Si vous utilisez `MCPStreamableHTTPConfig`, épinglez `mistralai>=2.8.0` dans les dépendances de votre projet.
:::
...
**Transmettre les identifiants à un serveur Streamable HTTP**

Un serveur Streamable HTTP a généralement besoin d'identifiants à chaque requête : un jeton bearer protégeant le point de terminaison et/ou un jeton d'intégration par requête. Définissez-les depuis l'environnement du worker afin qu'aucun secret ne soit sérialisé dans l'[historique d'événements](/studio/workflows/getting-started/core_concepts/events) du workflow.

Utilisez `auth_token_env` pour le bearer du point de terminaison (le SDK lit le jeton brut depuis cette variable du worker et ajoute le schéma `Bearer`). Utilisez `header_mapping` pour tout autre en-tête secret par requête, en associant un nom d'en-tête HTTP à un nom de variable du worker :
...
Seuls les noms de variables sont sérialisés dans les paramètres d'activité et l'historique d'événements ; les valeurs secrètes sont lues depuis l'environnement du worker à l'intérieur de l'activité et n'en sortent jamais. Ne mettez que des valeurs non secrètes dans le champ statique `headers`. Si une variable de worker déclarée est absente de l'environnement, l'activité échoue immédiatement.

**Redirections**

Par défaut, le client ne suit pas les redirections HTTP (`follow_redirects=False`). Lors d'une redirection, `httpx` ne retire l'en-tête `Authorization` que sur les sauts cross-origin, si bien que les en-têtes secrets par requête ci-dessus (par exemple `Notion-Token`) seraient renvoyés à la cible de la redirection. Ne définissez `follow_redirects=True` que si votre serveur MCP dépend des redirections (par exemple `/mcp` -> `/mcp/`) et que vous faites confiance à chaque hôte vers lequel il peut rediriger.
...
Les activités des agents produisent les mêmes événements workflow/activité que tout autre workflow. Abonnez-vous en temps réel via [Streaming](/studio/workflows/building-workflows/streaming) et [Consuming Streaming Events](/studio/workflows/building-workflows/consuming_events), ou rejouez l’historique complet a posteriori.
