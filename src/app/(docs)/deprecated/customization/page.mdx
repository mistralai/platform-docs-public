...
There are several techniques (with varying levels of engineering complexity) available to steer model behavior within your application context. We recommend using the three methods below to do so:
1. **System prompt**
2. **Tune a model**
...
1. **Application tuning**, where you use a dataset of examples specified to the desired behavior of your application.
2. **Safety tuning**, where you use a dataset that specifies both example inputs that might result in unsafe behavior, along with the desired safe output in that situation.
...
- **Automated Evals**
    ...
    - **LLM-based**, where you use a different LLM like Mistral Large to evaluate or judge the output of your custom model.
