...
# Configuration
Mistral Vibe is configured via a `config.toml` file. It looks for this file first in `./.vibe/config.toml` and then falls back to `~/.vibe/config.toml`.
...
:::tip
Mistral Vibe requires a Codestral API key. Where you create it depends on your plan:
- **Le Chat Pro subscribers** (including Le Chat Pro plans purchased through our partners — Google, Apple, Free Mobile, Orange): create your API key from <AppLink href="https://console.mistral.ai/codestral/cli" app="studio">Codestral › API keys</AppLink>
- **Experiment or Scale plan (pay-as-you-go API)**: create your API key from <AppLink href="https://admin.mistral.ai/organization/api-keys" app="studio">Organization › API keys</AppLink>
:::

:::info[Pay-as-you-go beyond your Le Chat Pro budget]
Le Chat Pro plans include a monthly Vibe budget. Once exhausted, what happens next depends on a toggle available in the Admin Panel under <AppLink href="https://admin.mistral.ai/subscriptions" app="admin">Subscriptions › Chat subscription</AppLink>:
...
This toggle is **off by default** on all subscriptions and **cannot be enabled on Le Chat Pro plans purchased through partners** (Google, Apple, Free Mobile, Orange). On those plans, Vibe is disabled once the included budget is reached.
:::

Mistral Vibe supports multiple ways to configure your API keys:
1. **Interactive Setup (Recommended for first-time users)**: When you run Mistral Vibe for the first time or if your API key is missing, Mistral Vibe will prompt you to enter it. The key will be securely saved to `~/.vibe/.env` for future sessions.
...
3. **`.env` File**: Create a `.env` file in `~/.vibe/` and add your API keys:
   ...
   ```
   Mistral Vibe automatically loads API keys from `~/.vibe/.env` on startup. Environment variables take precedence over the `.env` file if both are set.

:::note
The `.env` file is specifically for API keys and other provider credentials. General Mistral Vibe configuration should be done in `config.toml`.
:::

<SectionTab as="h1" sectionId="customize-behavior">Customize Behavior</SectionTab>

### Custom System Prompts and Agent Configurations

You can customize Mistral Vibe's behavior by modifying prompts and agent configurations.
...
To use a custom agent, run Mistral Vibe with the `--agent` flag:
```bash
vibe --agent my_custom_agent
```

Mistral Vibe will look for a file named `my_custom_agent.toml` in the agents directory and apply its configuration.
...
We allow users to change providers and models behind Mistral Vibe, for this you need to edit the `config.toml` file as follows:

- Create a new Provider preset in the config file:
   ```python
   [[providers]]
   name = "openrouter" # the name of the provider for the Mistral Vibe config
...
- Create a new Model preset:
   ...
   alias = "devstral-openrouter" # the alias for Mistral Vibe to use the model
   temperature = 0.2
   input_price = 0.0
   output_price = 0.0
   ```
...
Models created are also accessible via Mistral Vibe directly with the `/config` command, allowing you to change the model on the fly.

<SectionTab as="h1" sectionId="mcp-server-configuration">MCP Server Configuration</SectionTab>

### Configuring MCP Servers

You can configure MCP (Model Context Protocol) servers to extend Mistral Vibe's capabilities. Add MCP server configurations under the `mcp_servers` section:
```python
...
Mistral Vibe supports continuing from previous sessions:
...
<SectionTab as="h1" sectionId="custom-vibe-home-directory">Custom Mistral Vibe Home Directory</SectionTab>

### Customize Home Directory

By default, Mistral Vibe stores its configuration in `~/.vibe/`. You can override this by setting the `VIBE_HOME` environment variable:
```bash
export VIBE_HOME="/path/to/custom/vibe/home"
```
This affects where Mistral Vibe looks for:
...
Mistral Vibe includes a trust folder system to ensure you only run the agent in directories you trust. When you first run Mistral Vibe in a new directory, it may ask you to confirm whether you trust the folder.
...
<SectionTab as="h1" sectionId="update-settings">Update Settings</SectionTab>

### Auto-Update

Mistral Vibe includes an automatic update feature that keeps your installation current. This is enabled by default.
