Il existe de nombreuses façons d'évaluer les sorties de LLM. Voici trois approches pour votre référence : inclure un score de confiance, introduire une étape d'évaluation, ou employer un autre LLM pour l'évaluation.

### Inclure un score de confiance

Nous pouvons inclure un score de confiance avec la sortie générée.
...
- **Sortie JSON** : pour faciliter les tâches en aval, le format de sortie JSON est fréquemment préféré. Nous pouvons activer le mode JSON en définissant response_format sur `{"type": "json_object"}` et en précisant dans le prompt que « You will only respond with a JSON object with the key Summary and Confidence. » Spécifier ces clés dans l'objet JSON est bénéfique pour la clarté et la cohérence.
- **Temperature plus élevée** : dans cet exemple, nous augmentons le score de température pour encourager le modèle à être plus créatif et à produire trois résumés générés qui diffèrent les uns des autres.

### Introduire une étape d'évaluation

Nous pouvons également ajouter une seconde étape dans le prompt pour l'évaluation. Nous demandons au LLM de générer trois résumés et d'évaluer ces trois résumés.
...
### Employer un autre LLM pour l'évaluation

Dans les systèmes de production, il est courant d'employer un autre LLM pour l'évaluation afin que l'étape d'évaluation puisse être séparée de la génération des résumés.

- **Étape 1** : utiliser le premier LLM pour générer trois résumés
...
- **Étape 2** : utiliser un autre LLM pour évaluer les résumés générés
...
- **Chaînage de LLM** : dans cet exemple, nous chaînons deux LLM en séquence, où la sortie du premier LLM sert d'entrée pour le second LLM. La méthode de chaînage de LLM peut être adaptée à vos cas d'usage spécifiques. Par exemple, vous pouvez choisir d'employer trois LLM en chaîne, où la sortie de deux LLM est transmise au troisième LLM. Bien que le chaînage de LLM offre de la flexibilité, il est important de considérer qu'il peut entraîner des appels API supplémentaires et potentiellement des coûts accrus.
