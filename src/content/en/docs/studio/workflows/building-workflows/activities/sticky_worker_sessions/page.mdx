...
The `sticky_to_worker` parameter does not apply to [local activities](/studio/workflows/building-workflows/activities/local_activities), which already run in the workflow worker process.

For a comparison across activity flavors (regular, sticky, or local), see [Choosing an activity flavor](/studio/workflows/building-workflows/activities#choosing-an-activity-flavor) on the Activities parent page.
