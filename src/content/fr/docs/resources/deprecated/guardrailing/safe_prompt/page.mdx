...
:::warning
`safe_prompt` est obsolète. Nous recommandons d'utiliser les [garde-fous personnalisés](/studio/safety-moderation#custom-guardrails) à la place, qui offrent un meilleur contrôle sur les catégories de modération et les seuils directement dans vos requêtes API.
:::
...
:::tip
Avant de continuer, nous vous recommandons de lire la documentation [Chat Completions](/studio/conversations/chat-completion) pour en savoir plus sur l'API de complétion de conversation et comment l'utiliser avant de poursuivre.
:::
