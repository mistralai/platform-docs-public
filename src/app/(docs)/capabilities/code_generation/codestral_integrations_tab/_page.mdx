...
**Here's a step-by-step guide on how to set up Codestral with Continue using our API:**
...
- Click on the Continue extension icon on the left menu. Select `Mistral API` as a provider, select `Codestral` as a model.
...
- Log in and request a Codestral API key on our platform [here](https://console.mistral.ai/codestral)
...
Tabnine Chat is a code-centric chat application that runs in the IDE and lets developers
 interact with Tabnine’s AI models in a flexible, free-form way using natural language.
...
LangChain provides support for Codestral Instruct. Here's how you can use it in LangChain:
...
LlamaIndex provides support for Codestral Instruct and Fill In Middle (FIM) endpoints. Here's how you can use it in LlamaIndex:

```py
# make sure to install `llama-index` and `llama-index-llms-mistralai` in your Python environment
...
Jupyter AI integrates Codestral into JupyterLab, providing an AI-assisted coding experience within the Jupyter ecosystem.
...
JupyterLite is a project that brings the JupyterLab environment to the web browser, allowing users to run Jupyter directly in their browser without a local installation.
...
To use Codestral in Tabby, set up your model in `~/.tabby/config.toml` as follows.
...
With E2B, developers can easily add code interpreting capabilities to AI apps using Codestral.
