...
Un fichier batch pour les embeddings, avec `v1/embeddings` comme point de terminaison (Endpoint) et `mistral-embed` comme modèle, ressemblerait à ceci :
...
En tant que fichier JSONL, chaque ligne représente une requête vers le point de terminaison et le modèle de l'API.

<SectionTab as="h2" variant="secondary" sectionId="embeddings-explained">Explication</SectionTab>

Le corps de la requête suivra le même format que le point de terminaison sur lequel vous souhaitez exécuter votre traitement par batch, à l'exception de l'identifiant du modèle qui ne sera fourni que lors de la création du job pour lancer l'exécution du batch. Ci-dessous, nous fournissons un exemple de ligne avec des embeddings :
...
```py
{
    "custom_id": "2", # Un ID de métadonnée qui sera retourné dans le fichier de sortie pour identifier la requête
    "body": { # Le corps de la requête
        "input": [ # Le texte d'entrée à vectoriser, peut être une chaîne de caractères ou un tableau de chaînes
...
```py
{
  "id": "batch-089b0faa-3-9b701be0-78c3-4066-9b9c-8746db3d5fee", # L'ID de la requête
  "custom_id": "2", # L'ID personnalisé fourni en métadonnée dans le fichier d'entrée
...
Pour plus d'informations sur les embeddings, consultez la documentation [Embeddings](embeddings) et la [spécification API](../api/endpoint/embeddings#operation-embeddings_v1_embeddings_post) correspondante.
