...
Retrieval-augmented generation (RAG) is an AI framework that combines the capabilities of LLMs and information retrieval systems. It's useful to answer questions or generate content using external knowledge. There are two main steps in RAG:
...
This section guides you through the process of building a basic RAG from scratch. We have two goals: first, to give you a clear understanding of how RAG works internally; second, to provide you with the foundations needed to build a RAG using the minimum required dependencies.
...
To create an embedding, use our embeddings API endpoint and the embedding model `mistral-embed`. We create a `get_text_embedding` to get the embedding from a single text chunk and then we use list comprehension to get text embeddings for all text chunks.
...
Once we get the text embeddings, a common practice is to store them in a vector database for efficient processing and retrieval. There are several vector databases to choose from. In our simple example, we're using an open-source vector database Faiss, which allows for efficient similarity search.
