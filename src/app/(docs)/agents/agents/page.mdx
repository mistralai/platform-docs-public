...
Agents let you create predefined models with their own system prompts and tools. On the other hand, Conversations let you create a history of interactions with an assistant that uses Agents or our models directly.
...
For full details, visit our [Agents](https://docs.mistral.ai/api/#tag/beta.agents) and [Conversations](https://docs.mistral.ai/api/#tag/beta.conversations) API spec.
...
On the other hand, Conversations are a history of interactions with an assistant. They're more flexible and expressive than the Chat Completion API, allowing for more control over describing events.
...
You can also **opt out** of automatic storing with `store=False`; the new history won't be stored on our cloud.
...
For more information, visit our [handoffs documentation](../agents/handoffs).
