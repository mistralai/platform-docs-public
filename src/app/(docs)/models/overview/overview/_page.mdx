...
- `mistral-small-latest`: currently points to `mistral-small-2603`.
...
- Retirement date: The date to mark the model as retired. When a model is retired from Studio, it is no longer available for use, and when prompted, it will return an error response.
