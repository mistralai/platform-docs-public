...
## Our benchmarks
Our models demonstrate strong reasoning capabilities and perform well in advanced reasoning, multilingual tasks, math, and code generation. We report benchmark results on popular public benchmarks such as MMLU (Massive Multitask Language Understanding), MT-bench, and others.

You can find the benchmark results in the following blog posts: 
- [Magistral Medium](https://mistral.ai/news/magistral/): Our frontier-class reasoning model built on top of Mistral Medium 3, offering strong reasoning capabilities for enterprise use cases.
...
- [Mistral Medium](https://mistral.ai/news/mistral-medium-3/): A multimodal model.
...
- [Pixtral 12B](https://mistral.ai/news/pixtral-12b/): Pixtral 12B is the first open-source model to demonstrate strong multimodal understanding, without regressing on abilities in pure text.
- [Mistral Large](https://mistral.ai/news/mistral-large-2407/): A text generation model with strong reasoning capabilities.
...
- [Mistral Saba](https://mistral.ai/news/mistral-saba/): A powerful and efficient text generation model trained extensively on languages from the Middle East and South Asia, achieving quality comparable to much larger models.
It can be used for complex multilingual reasoning tasks, including text understanding, transformation, and code generation.
- [Mistral Nemo](https://mistral.ai/news/mistral-nemo/): Mistral Nemo's reasoning, world knowledge, and coding performance are leading in its size category. As it relies on standard architecture, Mistral Nemo is easy to use and a drop-in replacement in any system using Mistral 7B that it supersedes.
...
- [Codestral-Mamba](https://mistral.ai/news/codestral-mamba/): We trained this model with advanced code and reasoning capabilities, enabling strong performance on par with leading transformer-based models.
- [Mathstral](https://mistral.ai/news/mathstral/): Mathstral stands on the shoulders of Mistral 7B and specialises in STEM subjects. It achieves leading reasoning capacities in its size category across various industry-standard benchmarks.
...
[Scale AI](https://scale.com/leaderboard) reports private benchmark results in coding, instruction following, math, and Spanish. Mistral Large performs well in code and Spanish, outperforming Llama 3 405B in these areas.

## Artificial Analysis

[Artificial Analysis](https://artificialanalysis.ai/models) compares and evaluates AI models across key performance metrics, including quality, price, output speed, latency, context window, and others. Here are some notable results for our models:

- Artificial Analysis Quality Index: Our model ranks 3rd in this benchmark, surpassing the 405B model.
- Coding (HumanEval): In the HumanEval benchmark, our model ranks 3rd, again outperforming the 405B model.
- Quantitative Reasoning (MATH): Our model places 4th in the MATH benchmark, ahead of the 405B model.
- Scientific Reasoning & Knowledge (GPQA): In the GPQA benchmark, our model ranks 4th, demonstrating strong scientific reasoning and knowledge retention.
