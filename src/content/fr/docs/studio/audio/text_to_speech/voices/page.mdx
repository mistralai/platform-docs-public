...
Enregistrez des échantillons audio comme voix réutilisables. Une fois créée, une voix peut être référencée par `voice_id` dans toute requête de [génération de parole](/studio/audio/text_to_speech/speech), évitant ainsi de passer `ref_audio` à chaque fois.
