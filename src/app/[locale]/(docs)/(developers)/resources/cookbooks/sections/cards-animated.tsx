...
const useCardTitles = () => {
  const l = useLingo();
  return [
    l.text('Quick access to model recipes', { context: 'Short title for a featured cookbook topic' }),
    l.text('Advanced AI capabilities', { context: 'Short title for a featured cookbook topic' }),
    l.text('Code generation tools', { context: 'Short title for a featured cookbook topic' }),
    l.text('Language understanding', { context: 'Short title for a featured cookbook topic' }),
    l.text('Creative writing assistant', { context: 'Short title for a featured cookbook topic' }),
    l.text('Data analysis helper', { context: 'Short title for a featured cookbook topic' }),
  ];
};

const generateRandomCard = (id: number, colorIndex: number, cardTitles: string[]) => {
...
export const CardsAnimated = ({ initialCards }: CardsAnimatedProps) => {
  ...
  const cardTitles = useCardTitles();

  const advanceToNext = () => {
    setCards(prevCards => {
      const newCards = [...prevCards.slice(1)];
      const newCard = generateRandomCard(cardIdCounter, colorIndex, cardTitles);
