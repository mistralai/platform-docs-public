...
<SectionTab as="h1" sectionId="starting-a-worker">Démarrage d'un worker</SectionTab>
...
La variable d'environnement `MISTRAL_API_KEY` détermine à quel espace de travail le worker se connecte. Tous les workers partageant la même clé rejoignent le [déploiement](/studio-api/workflows/getting-started/core_concepts/deployments) nommé par `DEPLOYMENT_NAME` et reçoivent les exécutions routées vers ce déploiement.

```bash
MISTRAL_API_KEY=your_key DEPLOYMENT_NAME=invoice-service uv run python my_worker.py
```

<SectionTab as="h1" sectionId="execution-flow">Flux d'exécution</SectionTab>
...
2. La plateforme planifie une tâche de workflow pour le déploiement.
3. Un worker disponible récupère la tâche et commence l'exécution du workflow.
4. Lorsque le workflow appelle une activité, la plateforme planifie une tâche d'activité pour le déploiement.
...
<SectionTab as="h1" sectionId="worker-info">Infos worker</SectionTab>
