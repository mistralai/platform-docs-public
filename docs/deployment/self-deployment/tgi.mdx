...
Make sure to set the `HUGGING_FACE_HUB_TOKEN` environment variable to your [Hugging Face user access token](https://huggingface.co/docs/hub/security-tokens). This will spawn a TGI instance exposing an OpenAI-like API, as documented in the [API section](/api).  

If the model does not fit in your GPU, you can use quantization methods as well (AWQ, GPTQ, etc). You can find all TGI launch options at [their documentation](https://huggingface.co/docs/text-generation-inference/en/basic_tutorials/launcher). 
...
```
  ...
  <TabItem value="opena" label="Using OpenAI Client" default>
...
```
  ...
  <TabItem value="javascript" label="Using JavaScript" default>
