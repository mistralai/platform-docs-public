...
The Mistral plugin provides native integration with Mistral's AI models and services, including [durable agents](/studio/workflows/building-workflows/durable_agents), [tool calling](/studio/workflows/building-workflows/durable_agents#built-in-tools), and [multi-agent handoffs](/studio/workflows/building-workflows/durable_agents#multi-agent-handoffs):

```bash
uv add "mistralai-workflows[mistralai]"
```

For [payload offloading](/studio/workflows/building-workflows/payload_offloading) and direct cloud storage access from activities, install the extra for your provider:
