...
<SectionTab as="h2" variant="secondary" sectionId="streamable-http-mcp-server">Streamable HTTP MCP Server</SectionTab>
...
A Streamable HTTP server usually needs credentials on every request: a bearer token gating the endpoint and/or a per-request integration token. Set them from the worker's environment so no secret is serialized into the workflow's [event history](/studio-api/workflows/getting-started/core_concepts/events).
