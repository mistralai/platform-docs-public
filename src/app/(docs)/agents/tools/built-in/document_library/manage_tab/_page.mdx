...
We provide a full API for managing your libraries, from uploading, deleting and controlling access.

**Library Management Overview:**
- [Library Creation](#library-creation): Create a new library.
  - [Listing Libraries](#listing-libraries): List your existing libraries.
- [Upload Documents](#upload): Upload documents to a library.
  - [Listing Documents](#listing-documents): List documents in a library.
  - [Document Status](#status): Check the status of a document.
  - [Get Document](#get-document): Get document information.
  - [Get Document Content](#get-document-content): Get a document text content.
- [Delete Documents/Libraries](#delete-libraries): Delete documents or libraries.   
- [Control Access](#control-access): Control who has access to your libraries.

<SectionTab as="h2" variant="secondary" sectionId="library-creation">Library Creation</SectionTab>
...
Once created, the library will contain different kinds of information. This information is updated when files are added. Specifically, `generated_name` and `generated_description` are automatically kept up to date.
...
At first, a new library won't have any documents inside -- it's **empty by default**.

<SectionTab as="h3" variant="secondary" sectionId="listing-libraries">Listing Libraries</SectionTab>
...
<SectionTab as="h2" variant="secondary" sectionId="upload">Upload Documents</SectionTab>

You can upload documents to a library by providing the library ID and the document file.
...
<SectionTab as="h3" variant="secondary" sectionId="listing-documents">Listing Documents</SectionTab>
...
<SectionTab as="h3" variant="secondary" sectionId="status">Status</SectionTab>  

After uploading a document, you can check the status while it is being processed.
...
The status will be `Running` while the document is being processed and `Completed` once the document is ready to be used.
...
<SectionTab as="h3" variant="secondary" sectionId="get-document">Get document</SectionTab>

You can get the document information once it is processed.
...
<SectionTab as="h3" variant="secondary" sectionId="get-document-content">Get Document Content</SectionTab>

You can extract the text content from any document that belongs to a library.
...
<SectionTab as="h2" variant="secondary" sectionId="delete-libraries">Delete Libraries</SectionTab>

You can delete libraries and documents freely as needed.
...
<SectionTab as="h2" variant="secondary" sectionId="control-access">Control Access</SectionTab>

You can manage and control who has access to which libraries.  
This control is managed via different parameters:  
- `org_id` corresponds to the ID of your organization.
- `level` corresponds to the access level of the entity and can be one of two options: "Viewer" or "Editor".
- `share_with_uuid` corresponds to the ID of the entity you want to share with; you can find these in the console and platform settings.
...
- A Viewer can't edit libraries, unlike an Editor, who has permission to do so.
...
You can manage your libraries using our API. We recommend taking a look at the [API spec](https://docs.mistral.ai/api/#tag/beta.libraries.documents) for details. Below are some examples of how to interact with libraries and documents.
