...
- Prompt you to enter your API key if it's not already configured. You can create one from the [Mistral console](https://console.mistral.ai/codestral/cli).
...
You can run Vibe non-interactively by piping input or using the `--prompt` flag. This is useful for scripting, it allows you to run Vibe in a more streamlined manner. This won't enter the interactive chat loop interface.
...
You can also cap the number of steps or set a maximum session cost, and control general permissions and usage in the config file. Learn more in the [Configuration](configuration) guide.
...
You’re ready to use Mistral Vibe in your terminal. Start with small tasks, try the interactive commands, and build from there.
