...
const AnimatedModels = React.forwardRef<HTMLDivElement, { models: Model[]; l: Lingo }>(
  function AnimatedModels({ models, l }, ref) {
    ...
    <div className={cn('w-full', className)}>
      ...
      <div className="relative overflow-clip border border-secondary rounded-lg p-1 flex items-center w-full">
        <AnimatePresence mode="popLayout" custom={direction} initial={false}>
          <AnimatedModels key={currentPage} models={currentModels} l={l} />
        </AnimatePresence>
      </div>
      {totalPages > 1 && (
        <>
