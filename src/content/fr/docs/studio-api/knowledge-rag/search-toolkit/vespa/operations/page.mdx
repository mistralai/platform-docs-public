---
title: Déployer et exploiter
sidebar_position: 6
sidebar_label: Déployer et exploiter
---
import { SectionTab } from '@/components/layout/section-tab';

Déploiement en production et exploitation des applications Vespa.

<SectionTab as="h2" sectionId="vespaclientconfig">VespaClientConfig</SectionTab>

Configurez le backend de recherche avec les paramètres de connexion, délais d'expiration et politiques de réessai :

```python
from mistralai.search.toolkit.plugins.vespa import VespaClientConfig
from vespa_app import app

collection_name = "my_collection"
config = VespaClientConfig(
    endpoint="http://localhost:8080",
    # Options avancées
    timeout=30,              # Délai d'expiration de la requête, en secondes
    max_retries=3,          # Réessayer les requêtes échouées
    verify_ssl=True,        # Vérifier les certificats TLS
)

vector_store = app.get_search_index(config, collection_name=collection_name)
```

**Paramètres de connexion** :

| Paramètre       | Description                                 | Par défaut |
|-----------------|---------------------------------------------|------------|
| `endpoint`      | URL de l'endpoint de requête Vespa           | Obligatoire |
| `timeout`       | Délai d'expiration de la requête (secondes) | 30         |
| `max_retries`   | Nombre de tentatives automatiques sur échec  | 3          |
| `verify_ssl`    | Vérifier les certificats TLS                 | True       |

**Configuration TLS/SSL** (production) :

```python
from mistralai.search.toolkit.plugins.vespa import VespaClientConfig
from vespa_app import app

config = VespaClientConfig(
    endpoint="https://vespa.example.com:8080",
    verify_ssl=True,
    # Certificat CA personnalisé
    ca_cert_path="/path/to/ca-bundle.pem",
)

vector_store = app.get_search_index(config, collection_name="my_collection")
```

<SectionTab as="h2" sectionId="health-checks-and-readiness">Vérification d'état et préparation</SectionTab>

Avant d'ingérer ou d'interroger, vérifiez que Vespa est prêt :

```python
import asyncio
from mistralai.search.toolkit.plugins.vespa import VespaClientConfig
from vespa_app import app

async def wait_for_vespa(endpoint: str, max_attempts=30):
    """Attend que Vespa soit prêt à fonctionner."""
    for attempt in range(max_attempts):
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{endpoint}/ApplicationStatus",
                    timeout=5
                ) as resp:
                    if resp.status == 200:
                        print("Vespa est prêt")
                        return True
        except Exception as e:
            print(f"Tentative {attempt + 1} : {e}")
        await asyncio.sleep(2)

    raise RuntimeError("Vespa ne devient pas prêt")

# Utilisation
endpoint = "http://localhost:8080"
await wait_for_vespa(endpoint)
config = VespaClientConfig(endpoint=endpoint)
vector_store = app.get_search_index(config, collection_name="my_collection")
```

**Endpoints de vérification d'état** :

```bash
# Statut de l'application
curl http://localhost:8080/ApplicationStatus

# Nombre de documents dans la collection
curl "http://localhost:8080/search/?yql=select%20*%20from%20my_collection%20limit%201"
```

<SectionTab as="h2" sectionId="production-deployment">Déploiement en production</SectionTab>

Pour des environnements de production, suivez ces recommandations :

**Configuration du cluster** :
- Déployez au moins 3 nœuds Vespa pour la redondance
- Répartissez les nœuds entre différentes zones de disponibilité
- Placez un load balancer, avec vérification d'état, devant les endpoints de requête

**Configuration réseau** :
- Utilisez des réseaux privés pour la communication entre nœuds
- Exposez l'endpoint de requête via un reverse proxy (nginx, Cloudflare, etc.)
- Mettez en place une limitation de débit et une protection DDoS

**Exemple de configuration** (déploiement distribué) :

```python
from mistralai.search.toolkit.plugins.vespa import VespaClientConfig
from vespa_app import app

# Endpoint du load balancer (point d'entrée unique)
config = VespaClientConfig(
    endpoint="https://vespa-lb.example.com",
    timeout=30,
    max_retries=3,
    verify_ssl=True,
)

vector_store = app.get_search_index(config, collection_name="my_collection")
```

**Supervision** :
- Suivez la latence et le débit d'ingestion
- Surveillez la latence des requêtes et le taux d'erreurs
- Configurez des alertes en cas d'échec de la vérification d'état
- Surveillez l'utilisation du disque et la croissance de la taille de l'index

<SectionTab as="h2" sectionId="see-also">Voir aussi</SectionTab>

- **[Développement local](local-development)** — Guide d'installation locale
- **[Gestion via les migrations](migrations)** — Créer et faire évoluer les schémas
- **[Documentation officielle Vespa](https://docs.vespa.ai)** — Configuration avancée et exploitation
