...
:::warning[Knowledge Connectors are being deprecated]
Knowledge Connectors for Google Drive and SharePoint are scheduled for removal at the end of August 2026. Connect these services with the [MCP Connectors](/vibe/work/connectors/mcp-connectors) instead, which are available now. After the removal, an administrator needs to add the MCP Connectors and each user needs to reconnect their account.
:::
