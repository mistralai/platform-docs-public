...
The compromised npm packages are **inoffensive**. `Setup.mjs` references `tanstack_runner.js` but the payload file is named `router_init.js` making it useless. We still recommend removing them if you are impacted (see [Detection](#detection)).

## PyPi

The compromised PyPi package runs on a malicious script on import. It spawns a background process to harvest credentials from common locations. If you are impacted (see [Detection](#detection)) you should consider all host-accessible credentials to be leaked.
...
- 12/05/2026 - 15:09 CET: update the severity of NPM following our internal findings