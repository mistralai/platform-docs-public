...
The body request will follow the same format as the endpoint you want to run your batching, except the model ID, which is provided only during job creation to start the batch run. Below is an example row for OCR:
