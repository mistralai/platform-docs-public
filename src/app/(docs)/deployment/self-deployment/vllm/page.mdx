...
# vLLM
[vLLM](https://github.com/vllm-project/vllm) is an open-source LLM inference and serving engine. It's particularly appropriate as a target platform for self-deploying Mistral models on-premise.
...
- By default, vLLM sources the model weights from Hugging Face. To access Mistral model repositories, you need to be authenticated on Hugging Face, so an access token `HF_TOKEN` with the `READ` permission is required. You should also ensure that you've accepted the conditions of access on each model card page.
- If you already have the model artifacts on your infrastructure, you can use them directly by pointing vLLM to their local path instead of a Hugging Face model ID. In this scenario, you can skip all Hugging Face-related setup steps.

<SectionTab as="h1" sectionId="getting-started">Getting started</SectionTab>

The following sections guide you through deploying and querying Mistral models on vLLM.
...
If you're looking to deploy vLLM as a containerized inference server, you can use the project's official Docker image (see more details in the [vLLM Docker documentation](https://docs.vllm.ai/en/latest/serving/deploying_with_docker.html)).
...
Once the container is up and running, you can run inference on your model using the same code as in a standalone deployment.