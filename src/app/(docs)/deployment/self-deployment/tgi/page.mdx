...
Text Generation Inference (TGI) is a toolkit for deploying and serving large language models (LLMs). TGI enables high-performance text generation for the most popular open-access LLMs. Among other features, it supports quantization, tensor parallelism, token streaming, continuous batching, flash attention, guidance, and more.
The easiest way to get started with TGI is using the official Docker container.
...
If the model doesn't fit in your GPU, you can also use quantization methods (AWQ, GPTQ, etc.). You can find all TGI launch options in the [TGI documentation](https://huggingface.co/docs/text-generation-inference/en/basic_tutorials/launcher).
