...
| Vibe Code Web            | Activé      | Uniquement dans des déploiements dédiés                                        |
...
Pour plus de détails, consultez la [section sur le masquage du README du SDK](https://github.com/mistralai/client-ts#redaction) et les [exemples d’observabilité](https://github.com/mistralai/client-ts/tree/main/examples/src/observability).
...
Sur les workers que vous exécutez vous-même, définissez directement cette variable. Pour les workers hébergés par Mistral, la politique par défaut est appliquée pour vous et n'est pas configurable, sauf dans les déploiements dédiés où le niveau est défini pour l'ensemble du déploiement.
...
Les traces Vibe Work et Vibe Code Web sont expurgées selon la stratégie par défaut avant de quitter l'infrastructure de Mistral. Dans le service hébergé par Mistral, cette opération est gérée pour vous et n'est pas configurable. Dans un déploiement dédié, le niveau d'expurgation est défini pour l'ensemble du déploiement.
...
<Faq>
  <FaqItem question="Le masquage est-il activé par défaut ?">
    Oui, partout où Mistral contrôle le pipeline d'export : les SDK en mode dédié, Workflows,
    Vibe Code CLI, Vibe Code Web et Vibe Work. Si vous exécutez votre propre pipeline OpenTelemetry, encapsulez votre exporter
    avec `RedactingSpanExporter` pour l'activer.
  </FaqItem>
