...
You can create an agent with access to the document library by providing it as one of the tools. Note that you can still add more tools to the agent. The model is free to access and use the knowledge from the uploaded documents.
...
:::tip
It's also possible to specify libraries created via le Chat; these IDs are visible in the URL of the corresponding library created on le Chat, for example: `https://chat.mistral.ai/libraries/<library_id>`. To enable the agent to access a le Chat library, you need to be an Org admin and share it with the Organization.

The opposite is also possible: you can create a library via API and share it with your team on le Chat.
Learn more about [sharing and managing libraries](?tab=manage-libraries#explorer-tabs-agent_creation_and_library_management).
:::
