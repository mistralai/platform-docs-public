...
The `MISTRAL_API_KEY` environment variable determines which workspace the worker connects to. All workers sharing the same key join the [deployment](/studio-api/workflows/getting-started/core_concepts/deployments) named by `DEPLOYMENT_NAME` and receive executions routed to that deployment.
...
1. A workflow execution is triggered (via API, schedule, or Studio).
2. The platform schedules a workflow task for the deployment.
3. An available worker picks up the task and starts executing the workflow.
4. When the workflow calls an activity, the platform schedules an activity task for the deployment.
