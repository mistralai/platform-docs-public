...
With text embeddings, texts with similar meanings or context tend to be located closer to each other within this space, as measured by the distance between their vectors. This is because the model has learned to group semantically related texts together during training.

Let's look at a simple example. To simplify working with text embeddings, we can wrap the embedding API in this function:
...
We want to find how similar each code snippet is to the reference code **palindrome number**. We can see that the distance between the reference code embeddings and the **reverse** embeddings is smaller than the distance between the reference code embeddings and the **two sum** code embeddings.
...
In our example above, we used the Euclidean distance to measure the distance between embedding vectors (note that since our embeddings are norm 1, cosine similarity, dot product, or Euclidean distance are all equivalent).