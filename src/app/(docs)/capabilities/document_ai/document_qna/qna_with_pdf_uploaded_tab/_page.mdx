...
You can also upload a PDF file to our cloud and get the QnA results from the uploaded PDF by retrieving a signed URL. Document QnA is under the umbrella of OCR, so the method for uploading and handling files is the same.

<SectionTab as="h3" variant="secondary" sectionId="upload-a-file">Upload a file</SectionTab>

First, upload your PDF file to our cloud. The file will be stored and only accessible via an API key.
...
Once the file is uploaded, you can retrieve it at any point.
...
For QnA with documents, you can get a signed URL to access the file. An optional `expiry` parameter lets you automatically expire the signed URL after n hours.
...
Once everything is done, you can optionally delete the PDF file from our cloud unless you wish to reuse it later.
