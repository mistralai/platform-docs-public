...
You can fine-tune all of our models via the Mistral API. Follow the steps below using our fine-tuning API.
...
If you upload this `ultrachat_chunk_train.jsonl` to the Mistral API, you might encounter an “Invalid file format” error due to data formatting issues. To reformat the data into the correct format, you can download the reformat_data.py script and use it to validate and reformat both the training and evaluation data:
...
:::important[ ]
This `reformat_data.py` script is tailored for the UltraChat data and may not apply to other datasets. Please modify this script and reformat your data accordingly.
:::
...
### Analyze and evaluate the fine-tuned model

When you retrieve a model, you get the following metrics every 10% of the progress with a minimum of 10 steps in between:
...
We also support integration with Weights & Biases (W&B) to monitor and track various metrics and statistics associated with fine-tuning jobs. To enable integration with W&B, you'll need to create an account with W&B and add your W&B information in the “integrations” section in the job creation request:
...
We’ve also open-sourced the fine-tuning codebase mistral-finetune, allowing you to fine-tune our open-weight models (Mistral 7B, Mixtral 8x7B, Mixtral 8x22B).
