...
Une fois qu’un Connecteur est [enregistré](/studio/connectors/management) et [authentifié](/studio/connectors/management#get-auth-url) (si nécessaire), vous pouvez l’attacher à n’importe quelle conversation.
...
:::tip
Vous pouvez exiger une approbation de l’utilisateur avant l’exécution d’un outil en ajoutant `requires_confirmation` à la `tool_configuration`. Cela est utile pour des actions sensibles comme l’envoi d’[e-mails](/studio/connectors/confirmation) ou la modification de données. Voir [Validation humaine](/studio/connectors/confirmation) pour le flux complet de confirmation.
:::

<SectionTab as="h1" sectionId="combining-tools">Utiliser les outils intégrés avec les Connecteurs</SectionTab>

Mistral fournit des [outils intégrés](/studio/agents/agent-tools) tels que `web_search`, `code_interpreter`, `image_generation` et `document_library`. Vous pouvez les passer aux côtés des Connecteurs dans le même tableau `tools`. Le modèle décide quel outil appeler **en fonction de la requête**.

:::warning
`web_search`, `web_search_premium` et `code_interpreter` fonctionnent avec l'[API Conversations](/studio/connectors/conversations) (`/v1/conversations`) et l'API Agents. Ils ne sont **pas pris en charge** dans l'API Chat Completions (`/v1/chat/completions`). Les outils intégrés comme `image_generation` sont aussi pris en charge dans l'API Chat Completions.
:::
