...
### Code samples (three layers, later wins)

1. **docs-md SDK generation** — python/TS samples generated by docs-md from the
   SDK tarballs listed in `speakeasy.config.mjs` (`sdks/client-python-2.5.2.tar.gz`,
   `sdks/client-ts-2.4.1.tar.gz`; V1 python from `client-python-1.9.11.tar.gz`
   via `speakeasy.v1.config.mjs`). Offline baseline, versioned in the repo.
2. **Curated code-sample DB** (`src/scripts/api-code-samples.v2.yaml`) —
   hand-checked `x-codeSamples` for specific operations (chat, OCR): curl with
   `$MISTRAL_API_KEY`, curated request bodies. Applied by `api:apply` like the
   examples patch DB.
3. **Speakeasy registry overlay** — `x-codeSamples` auto-published by the SDK
   repos (`client-python`, `client-ts`) on every `speakeasy run`, applied on top
   of the output spec by `applyCodeSampleOverlays()` via
   `speakeasy pull --spec <name>` + `overlay apply`. **Best-effort**: without
   the speakeasy CLI or authentication (`speakeasy auth login` /
   `SPEAKEASY_API_KEY`) it is skipped with a warning and the build falls back
   to layers 1-2. Silence with `API_CODE_SAMPLE_OVERLAYS=''`.

   CLI gotcha: `speakeasy pull` requires `--spec <name>`; passing the full
   registry location positionally fails **silently** (exit 1, no output) even
   when authenticated.

   Known limitation: both SDK repos publish to the same location with
   last-writer-wins on `latest`, so the pulled overlay contains ONE target
   per build (typescript or python, whichever repo published last). The
   overlay's target is logged. Until the SDK repos publish per-target
   locations (e.g. `...-code-samples-python` / `...-code-samples-typescript`),
   the other language comes from layers 1-2.

### Patch drift detection (`originalSha`)

Each patch can carry `originalSha` — the SHA-256 (canonical JSON) of the
upstream value it originally overwrote. `pnpm api:apply --record-shas` records
them into the patch DBs. On every apply:

- target already equals the patch value → **redundant**: upstream adopted the
  fix, the patch can be retired
- upstream value no longer matches `originalSha` → **drifted**: the spec
  changed underneath the patch, review the patch

Warnings are printed in the apply summary; neither fails the build (orphans
still do). Note: `pnpm api:promote` copies the draft over `api-examples.yaml`,
which drops recorded SHAs — re-run `--record-shas` after promoting.
...
| `pnpm api:apply`         | Build `.openapi-docs.yaml` from source + patches + transforms. `--record-shas` backfills `originalSha` for drift detection. |
...
api-code-samples.v1.yaml        V1 code-sample patch DB (currently empty)
api-code-samples.v2.yaml        curated V2 x-codeSamples (chat, OCR) — applied by api:apply
sdks/*.tar.gz                   SDK tarballs for docs-md sample generation (see layers above)
```
