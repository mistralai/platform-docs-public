...
Si vous avez une image ou un ensemble d'images stockées localement, vous pouvez les transmettre au modèle au **format encodé en base64**. L'encodage base64 est une méthode courante pour convertir des données binaires en format texte facilement transmissible sur Internet. C'est particulièrement utile lorsque vous devez inclure des images dans des requêtes API.
