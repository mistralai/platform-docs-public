...
:::warning
`web_search` et `web_search_premium` fonctionnent avec l'[API Conversations](/studio-api/connectors/conversations) (`/v1/conversations`) et l'API Agents. Ils ne sont **pas pris en charge** dans l'API Chat Completions (`/v1/chat/completions`), car les réponses Chat Completions n'incluent pas les références de résultats de recherche renvoyées par ces outils.
:::
...
- **`message.output`** : cette entrée correspond à la réponse générée par notre agent. Elle inclut des métadonnées sur le message, telles que :
  - `content` : le contenu réel du message, qui dans ce cas est une liste de fragments. Ces fragments comprennent le texte de réponse du modèle, entrelacé avec des fragments de référence. Les fragments de référence sont utilisés pour les citations lors des appels d'outils liés à la génération augmentée par récupération (RAG). Dans ce cas, ils indiquent la source des informations dans la réponse. La section `content` comprend :
...
Un autre outil qui utilise les références est l'outil Document Library. Pour plus de détails, consultez le [guide des bibliothèques](/studio-api/libraries#connecting-libraries-to-agents).
