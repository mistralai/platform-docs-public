...
export default function CookbookTopicsSection() {
  ...
  const exploreMistralApis = l.text('Explore the Mistral AI APIs', { context: 'Description of Mistral APIs on a cookbook topic card' });
  ...
  return (
    <section id="topics" className="flex flex-col gap-6">
      <SectionTab sectionId='topics'>{l.text('Topics', { context: 'Heading for cookbook topics' })}</SectionTab>

      <Heading align="between">
        <HeadingTitle size="h3" as="h2">
          {l.text('Explore by Topics', { context: 'Subheading for cookbook topics' })}
        </HeadingTitle>
        <HeadingSubtitle>
          {l.text('Discover our cookbooks by use case', { context: 'Description of browsing cookbooks by topic' })}
        </HeadingSubtitle>
      </Heading>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4">
        {displayUseCases.map(useCase => (
          <FeatureCard
            as="button"
            key={useCase}
            title={cookbookUseCaseLabel(useCase, l)}
            description={exploreMistralApis}
