...
1. Créez un **Connecteur Linear** dans [Studio › Contexte › Connecteurs](https://console.mistral.ai/build/connectors). Nommez-le `linear` pour qu’il corresponde au code du workflow.
...
Pour plus de détails, consultez [déploiements renforcés](/studio-api/workflows/managing-workflows-in-production/hardened_deployments) et [connecteurs](/studio-api/workflows/building-workflows/connectors).
...
Quand aucune entrée n’est fournie, le workflow marque une pause et invite à entrer l’équipe et le Projet dans [Studio](https://console.mistral.ai/build/workflows/linear-weekly-summary). Vous pouvez également fournir une entrée directement :
...
Ou lancez `linear-weekly-summary` dans [Studio](https://console.mistral.ai/build/workflows?workflowSearch=linear-weekly-summary) avec cette entrée :
