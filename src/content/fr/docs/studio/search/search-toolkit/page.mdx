...
[Ingestion](/studio/search/search-toolkit/ingestion) inclut :
...
[Récupération](/studio/search/search-toolkit/retrieval) inclut :
...
L’**ingestion** transforme des documents bruts en fragments exploitables pour la recherche. Un `FileLoader` lit les octets bruts depuis une source, un `DocumentExtractor` les convertit en document structuré, un `TextSplitter` le divise en fragments, des étapes `DocumentProcessor` facultatives (comme un `ChunkEnricher` ou un `SummaryEnricher`) ajoutent des métadonnées, et un `Embedder` produit des vecteurs pour les indexer dans une base vectorielle.
...
| **Extracteurs** | `MistralOCRExtractor`, `SmartPDFExtractor`, `PlainTextExtractor`, `PyMuPDFExtractor`, `HTMLExtractor`, `SpreadsheetExtractor`, `EmailExtractor`, `EmailWithAttachmentsExtractor`, `NumbersExtractor`, `LegacyOfficeExtractor`, `MistralAudioTranscriptionExtractor` |
| **Séparateurs de texte** | `CharacterTextSplitter`, `TokenTextSplitter`, `MarkdownTextSplitter`, `ParagraphTokenTextSplitter`, `MarkdownTokenTextSplitter`, `SeparatorTextSplitter` |
| **Enrichisseurs** | `SummaryEnricher`, `ChunkEnricher` personnalisé |
| **Embedders** | `MistralEmbedder`, `Embedder` personnalisé |
| **Stockage** | Vespa, Postgres (pgvector) ou stockage vectoriel personnalisé |
| **Récupérateurs** | `VectorRetriever`, `KeywordRetriever` avec un `KeywordStoreIndex` personnalisé |
...
| `postgres` | Backend PostgreSQL avec pgvector et pg_textsearch |
...
| `storage-s3` | Intégration Amazon S3 |
| `language-detection-fasttext` | Détection de la langue via fastText |
...
- **[Guide de démarrage](/studio/search/search-toolkit/quickstart)** : créez votre premier pipeline d’ingestion et de récupération de bout en bout.
- **[Modèle de document](/studio/search/search-toolkit/concepts/document-model)** : comprenez `Document`, `DocumentChunk` et l’identifiant qui les relie.
- **[Modèle d'embedding](/studio/search/search-toolkit/concepts/embedding-model)** : comprenez comment le schéma décrit les vecteurs d'embedding, indépendamment de l'embedder.
- **[Index de recherche](/studio/search/search-toolkit/search-index)** : configurez votre base vectorielle.
- **[Ingestion](/studio/search/search-toolkit/ingestion)** : chargez, extrayez, découpez en fragments, enrichissez et indexez vos documents.
- **[Récupération](/studio/search/search-toolkit/retrieval)** : configurez la recherche vectorielle avec reclassement optionnel.
