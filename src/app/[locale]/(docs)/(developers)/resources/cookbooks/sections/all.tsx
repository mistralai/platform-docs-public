...
import { cookbookUseCaseLabel } from '@/schema/cookbook/i18n';
...
function FilterTabsUI({
  ...
  const useCaseOptions: ComboboxOption[] = allUseCases.map(useCase => ({
    value: useCase,
    label: cookbookUseCaseLabel(useCase, l),
  }));
  ...
  return (
    <div className="w-full group/tabs flex gap-3 justify-start items-end border-b border-secondary">
      ...
      >
        ...
        >
          {l.text('All ({count})', { values: { count: totalCount }, context: 'Filter tab on the cookbooks table that clears all filters; shows the total cookbook count' })}
        </button>
        ...
        <Combobox
          ...
          placeholder={l.text('Integration', { context: 'Placeholder/label on the "Integration" filter tab for the cookbooks table' })}
          searchPlaceholder={l.text('Search integrations...', { context: 'Search placeholder inside the Integration filter dropdown on the cookbooks table' })}
          emptyText={l.text('No integration found.', { context: 'Empty-state text in the Integration filter dropdown on the cookbooks table' })}
          trigger={createTabTrigger(
            integrationFilter || l.text('Integration', { context: 'Placeholder/label on the "Integration" filter tab for the cookbooks table' }),
            !!integrationFilter
          )}
        ...
        <Combobox
          ...
          placeholder={l.text('Use Case', { context: 'Placeholder/label on the "Use Case" filter tab for the cookbooks table' })}
          searchPlaceholder={l.text('Search use cases...', { context: 'Search placeholder inside the Use Case filter dropdown on the cookbooks table' })}
          emptyText={l.text('No use case found.', { context: 'Empty-state text in the Use Case filter dropdown on the cookbooks table' })}
          trigger={createTabTrigger(
            useCaseFilter || l.text('Use Case', { context: 'Placeholder/label on the "Use Case" filter tab for the cookbooks table' }),
            !!useCaseFilter
          )}
          matchTriggerWidth={false}
          width="w-auto"
      ...
      <div className="relative ml-auto min-w-[200px]">
        ...
        >
          ...
          <input
            type="text"
            placeholder={l.text('Search', { context: 'Search input placeholder on the cookbooks table' })}
            value={searchQuery}
...
const RenderedFilteredCookbookTable = (
  ...
  return props.data.length > 0 ? (
    ...
    <div className="text-center py-12 text-muted-foreground">
      <p>{l.text('No cookbooks found. Try adjusting your filters or search terms.', { context: 'Empty-state shown in the cookbooks table when the active filters and search match nothing' })}</p>
    </div>
  );
};
