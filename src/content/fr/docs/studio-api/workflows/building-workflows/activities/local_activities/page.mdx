...
Les activités locales s'exécutent directement dans le processus worker du workflow, en passant outre les files d'attente de tâches du [déploiement](/studio-api/workflows/getting-started/core_concepts/deployments). Elles sont utiles pour des opérations rapides (moins d'une seconde) lorsque la surcharge de planification est significative.
