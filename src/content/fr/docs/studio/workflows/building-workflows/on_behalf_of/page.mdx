...
:::warning
Les workflows on-behalf-of nécessitent un [déploiement renforcé](/studio/workflows/managing-workflows-in-production/hardened_deployments). Ils ne peuvent pas être enregistrés dans un déploiement non renforcé.
:::
...
:::note
`get_mistral_client()` ne se limite pas à la prise en charge OBO : il gère aussi les métadonnées de télémétrie et d’observabilité. Consultez [client Mistral](/studio/workflows/building-workflows/mistral_client) pour la liste complète des fonctionnalités.
:::
