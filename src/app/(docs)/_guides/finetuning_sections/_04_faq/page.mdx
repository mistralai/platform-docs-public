...
<Tabs>
  ...
  However, these scripts might not detect all problematic cases. You may need to manually validate and correct any unique edge cases in your data.
  </FaqItem>
  <FaqItem question="What's the size limit of the training data?">

While the size limit for an individual training data file is 512 MB, there's no limitation on the number of files you can upload. You can upload multiple files and reference them when creating the job.
  </FaqItem>
  <FaqItem question="What's the size limit of the validation data?">

The size limit for the validation data is 1 MB. As a rule of thumb:
...
Mistral API: When you create a fine-tuning job, you should automatically see this info with the default `auto_start=False` argument.
