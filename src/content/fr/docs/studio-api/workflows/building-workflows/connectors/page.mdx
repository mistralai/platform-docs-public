...
:::warning
Les connecteurs ne sont utilisables que dans les [workflows on-behalf-of](/studio-api/workflows/building-workflows/on_behalf_of), qui nécessitent un [déploiement renforcé](/studio-api/workflows/managing-workflows-in-production/hardened_deployments). Consultez [Enregistrer un workflow OBO](/studio-api/workflows/managing-workflows-in-production/hardened_deployments#registering-an-obo-workflow) pour les étapes de configuration.
:::
...
- `on_behalf_of=True` exécute le workflow sous l'identité de l'appelant. Obligatoire pour la résolution d'identifiants par utilisateur.
