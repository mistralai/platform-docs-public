...
:::info
Vous voulez chiffrer ou compresser les payloads en plus de les externaliser ? Consultez [Chiffrement](/studio-api/workflows/building-workflows/encryption) et [Compression](/studio-api/workflows/building-workflows/compression). Les trois fonctionnalités peuvent être combinées. La compression s’exécute en premier et peut ramener un payload sous la limite de 2 Mo, ce qui évite entièrement l’externalisation.
:::
...
Définissez ces variables d’environnement sur vos workers. La présence du bloc de configuration de stockage active la fonctionnalité. Il n’y a pas d’option `ENABLED`. La taille minimale par défaut est 1 Mo ; vous pouvez l’ajuster avec `MIN_SIZE_BYTES`.
...
Comme pour le déport de champ, la présence du bloc de configuration active la fonctionnalité. Il n’y a pas d’option `ENABLED`.
