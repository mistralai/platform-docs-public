...
Vibe remembers the last tab you selected. If **Code** opens by default, select **Work** in the sidebar before starting the task.

{/* TODO: Add a screenshot showing the `Work` and `Code` tabs in the sidebar. */}

:::tip
Not sure which mode to use? See [Choose Work or Code](/vibe/choose-chat-work-code).
:::
...
:::note
Quick, turn-based conversations happen in the same place — switch the **fast/think toggle** to **fast**. Legacy Chat features have equivalents in Work: [Agents](/vibe/chat-legacy/agents) point to [Skills](/vibe/work/skills) (invoke them with `@`), [Memories](/vibe/chat-legacy/memories) map to the [Knowledge Base](/vibe/work/knowledge), and Deep Research is the [Deep Research Skill](/vibe/work/skills). [Code Interpreter](/vibe/work/code-interpreter) is now a native sandbox in Work, available on paid plans.
:::
