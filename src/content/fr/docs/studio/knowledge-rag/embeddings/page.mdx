...
Si vous souhaitez une fonctionnalité gérée qui ingère, vectorise et recherche des documents pour vous, utilisez [Bibliothèques](/studio/search/libraries). Si vous souhaitez rechercher des sources connectées comme Google Drive ou SharePoint, utilisez [Connecteurs](/studio/connectors).

<SectionTab as="h1" sectionId="services">Services</SectionTab>

Nous proposons deux modèles d’embeddings :

- [Embeddings de texte](/studio/knowledge-rag/embeddings/text_embeddings) : intégrez une large variété de texte avec un modèle d’embedding polyvalent.
- [Embeddings de code](/studio/knowledge-rag/embeddings/code_embeddings) : intégrez des bases de données de code et des dépôts pour la récupération de code.
...
Pour un exemple rapide et une introduction sur l’utilisation des embeddings pour le RAG, consultez [guide de démarrage RAG](/studio/knowledge-rag/rag_quickstart).
