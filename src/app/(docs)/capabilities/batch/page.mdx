...
For large batches of up to 1M requests, create a .jsonl file with the above data. Once saved, you can upload your batch input file to ensure it's correctly referenced when initiating batch processes.

:::tip
For batches with fewer than 10k requests, we support [inline batching](#inline-batching).
:::
...
You won't need to use the API to upload your files and/or create batching jobs.
...
The standard batching approach relies on batch files containing all the requests to be processed. We support up to 1 million requests in a single batch, enabling efficient handling of large volumes of requests at a reduced cost. This is ideal for tasks with high throughput requirements but low latency sensitivity.
...
Once the batch is sent, you'll want to retrieve information such as:
- The status of the batch job
- The results of the batch job
- The list of batch jobs

<SectionTab as="h2" variant="secondary" sectionId="get-a-batch-job-details">Get batch job details</SectionTab>
...
Once the batch job is complete, you can download the results.
...
We offer a 50% discount on the batch API. Learn more about our [pricing](https://mistral.ai/pricing#api-pricing).
  </FaqItem>
  <FaqItem question="Does the batch API affect rate limits?">
No
  </FaqItem>
  <FaqItem question="What's the max number of requests in a batch?">
Currently, there's a maximum limit of 1 million pending requests per Workspace. This means you can't submit a job with more than 1 million requests. Additionally, you can't submit two jobs with 600,000 requests each at the same time. You'd need to wait until the first job has processed at least 200,000 requests, reducing its pending count to 400,000. At that point, the new job with 600,000 requests would fit within the limit.
  </FaqItem>
...
Users can set `timeout_hours` when creating a job, which specifies the number of hours after which the job should expire. It defaults to 24 hours and should be lower than 7 days. A batch expires if processing doesn't complete within the specified timeout.
  </FaqItem>
...
No, the results don't expire at this time.
  </FaqItem>
