...
Voici un exemple d’utilisation de nos fonctionnalités d’annotation de documents.

<SectionTab as="h3" variant="secondary" sectionId="document-define-the-data-model">Définir le modèle de données</SectionTab>

Commencez par définir les formats de réponse pour l’`annotation de documents`, en utilisant soit des schémas Pydantic ou Zod pour nos SDK, soit un schéma JSON pour un appel d’API via curl.

Les schémas Pydantic, Zod et JSON acceptent les objets imbriqués, les tableaux, les énumérations, etc.
...
Vous pouvez également fournir une description pour chaque entrée. Cette description servira d’informations détaillées et d’instructions pendant l’annotation, par exemple :
...
```
    ...
    <TabItem value="curl" label="schéma JSON curl">
...
<SectionTab as="h3" variant="secondary" sectionId="document-annotation-prompt">Prompt d’annotation (facultatif)</SectionTab>

Après avoir défini votre schéma d’annotation, vous pouvez avoir besoin de fournir davantage de Contexte et d’instructions sur l’annotation et le document concerné. Nous permettons de transmettre un `document_annotation_prompt` qui servira de prompt système de haut niveau pour l’étape d’annotation, offrant un Contexte supplémentaire et des instructions sur la manière dont l’annotation doit être effectuée. Un exemple de prompt serait :
...
En production, vous pouvez souhaiter fournir un Contexte sur le cas d’usage et affiner davantage les instructions si la sortie ne correspond pas exactement aux attentes.

<SectionTab as="h3" variant="secondary" sectionId="document-start-request">Lancer la requête</SectionTab>

Ensuite, effectuez une requête et vérifiez que la réponse respecte les structures définies en utilisant `document_annotation_format` paramétré sur les schémas correspondants :
...
<SectionTab as="h3" variant="secondary" sectionId="document-example-output">Exemple de sortie d’annotation de document</SectionTab>

La fonctionnalité d’annotation de documents permet d’extraire des données et d’annoter des documents. Ci-dessous, un exemple de sortie d’annotation :
