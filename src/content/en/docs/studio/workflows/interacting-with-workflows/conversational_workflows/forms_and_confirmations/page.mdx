...
| --------------- | ---------------- | ------------------------------------------------------------------------------------------------- |
| `TextField`     | Text input       | `description`, `pattern` (optional regex), `prefilled_value`, `error_message`                     |
...
By default, a value that fails `pattern` is rejected with a generated message that quotes the regex back to the user, such as `^00\d{8}$`. Set `error_message` to replace it with wording the user can act on. The message is presentational only: it does not change what the field accepts. Unlike the generated message, it is shown as written rather than adapted to the user's language.

`error_message` also replaces the regex shown in the hint under the field.
