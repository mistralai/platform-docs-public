...
In other applications, you may need to detect and filter out harmful or PII (Personally Identifiable Information) content.
...
- **System prompt guardrailing**: An optional system prompt to enforce guardrails on top of our models to steer behavior and reduce harmful content.

<SectionTab as="h1" sectionId="moderation">Moderation</SectionTab>

### Moderate inputs/outputs

Our moderation service, powered by the Mistral Moderation model, is a classifier model
based on Ministral 8B 24.10. It lets you detect harmful text content along several policy dimensions.

<SectionTab as="h2" variant="secondary" sectionId="endpoints">Endpoints</SectionTab>

We provide two endpoints: one to classify **raw text** and one to classify **conversational content**. More details below.
...
:::note[ ]
The policy threshold is based on the optimal performance of our internal test set.
...
Custom policies that depend on `category_scores` may require recalibration.
:::
...
For a broader view, there is also a more [explorative cookbook](https://colab.research.google.com/github/mistralai/cookbook/blob/main/mistral/moderation/moderation-explored.ipynb).
...
The ability to enforce guardrails in chat generations is crucial for front-facing applications. We provide an optional system prompt to enforce guardrails on top of our models. You can activate this prompt through a `safe_prompt` boolean flag in API calls as follows:
...
To evaluate the model's ability to avoid inappropriate outputs, we used a set of adversarial prompts deliberately asking for content excluded by guardrails, kindly provided by the community.

With the recommended system prompt, our models decline to answer all the questions in the set of adversarial prompts we've tested.
...
Mistral models can also act as effective content moderators: the model itself can accurately classify a user prompt or its generated answer as being either acceptable or falling into one of the following categories:
