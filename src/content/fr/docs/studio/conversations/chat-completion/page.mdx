...
- Un `tool message` n'apparaît que dans le contexte de **l'appel de fonction**, il est utilisé à l'étape finale de formulation de la réponse lorsque le modèle doit formater la sortie de l'appel d'outil pour l'utilisateur. Pour en savoir plus sur l'appel de fonction, consultez le [guide](/studio/agents/agent-tools/function-calling).
...
:::tip
si vous êtes intéressé par une façon simplifiée de gérer les conversations multi-tour, vous pouvez consulter nos [API Agents et Conversations](/studio/agents/introduction). La gestion des conversations multi-tour peut être complexe, et nos API sont conçues pour simplifier ce processus tout en vous fournissant des outils et connecteurs intégrés.
:::
...
- Le flag `safe_prompt` est utilisé pour forcer la modération de la chat completion contre les contenus sensibles (voir [Guardrailing](/studio/safety-moderation)).
...
Ceci était une introduction simple à notre service de chat completion, cependant nous avons beaucoup plus à offrir et nous vous recommandons d'y jeter un œil ; des capacités de [Vision](/studio/conversations/vision), à l'[appel de fonction](/studio/agents/agent-tools/function-calling), les [sorties prédites](/studio/conversations/advanced/predicted-outputs), les [sorties structurées](/studio/conversations/structured-output) et bien plus encore.
