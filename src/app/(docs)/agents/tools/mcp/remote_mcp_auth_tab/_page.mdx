...
Here's how to use a remote MCP server with authentication.

<SectionTab as="h3" variant="secondary" sectionId="remote-auth-step-1">Step 1: Initialize the Mistral client</SectionTab>

First, we import everything needed. Most of the required modules are available with our `mistralai` package. All the MCP clients run asynchronously, so we'll create an async main function where the main code will reside.
