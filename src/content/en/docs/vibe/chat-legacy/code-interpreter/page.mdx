...
:::warning
Code Interpreter is a legacy Chat feature. The Chat tab is being removed progressively — Enterprise organizations may still have access during the six-month migration window.

Code Interpreter has been replaced by a native code interpreter in [Work](/vibe/work/code-interpreter), available to paid users with the same capabilities.
:::

Code Interpreter lets you **run Python and TypeScript directly inside Vibe**.
