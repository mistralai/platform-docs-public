...
`client.evaluation.retry_failed_records(...)` accepte les paramètres suivants :

| Paramètre | Type | Description |
|-----------|------|-------------|
| `run_id` | `str` | **Obligatoire.** ID de l'exécution contenant les enregistrements en échec. |
| `dataset` | `Sequence[Mapping[str, Any]]` | **Obligatoire.** Même dataset que celui utilisé dans l'exécution d'origine. |
| `task` | `TaskFunction` | **Obligatoire.** Fonction de tâche (peut être une version corrigée). |
| `evaluators` | `list[Evaluator]` | **Obligatoire.** Évaluateurs à utiliser pour recalculer les scores. |
| `run_evaluators` | `list[RunEvaluator]` | Évaluateurs au niveau de l'exécution à recalculer après correction. |
...
| `upload_batch_size` | `int` | Taille du batch pour télécharger les résultats corrigés (par défaut : `10`). |

Cette méthode renvoie un `RetryFailedRecordsResult` :

| Champ | Type | Description |
|-------|------|-------------|
| `run_id` | `str` | ID de l'exécution qui a été relancée. |
| `retried_count` | `int` | Nombre d'enregistrements qui ont été relancés. |
| `patched_count` | `int` | Nombre d'enregistrements corrigés avec succès. |
