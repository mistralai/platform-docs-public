...
> The output list is already slightly different from the first one, being impacted by the presence penalty of present tokens. For instance, we have less `The` as a token compared to without presence penalty.
