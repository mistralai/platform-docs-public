...
**Vibe Work already covers the same use case** through the [`/deep-research` Skill](/vibe/work/skills) combined with [Canvas](/vibe/work/files-and-canvas). That is the long-term shape of the feature.

The Chat implementation is **legacy**: it does not work with Connectors and is no longer actively developed. It stays available for continuity — no public deprecation date set, but expect it to be phased out as the Work Skill reaches parity.
:::
