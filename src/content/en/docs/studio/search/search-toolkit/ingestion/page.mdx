...
Optional `DocumentProcessor` stages (such as a `ChunkEnricher` or `SummaryEnricher`) add metadata to each chunk before indexing, for example an LLM-generated summary or entity tags.
...
from mistralai.search.toolkit.ingestion.enrichment import SummaryEnricher
...
pipeline = Pipeline(
    ...
    processors=[SummaryEnricher(llm_provider=llm)],
...
pipeline = Pipeline(
    ...
    embedder=embedder,
    stores=vector_store,
    checkpoint_store=lambda: FilesystemStorage("./checkpoints"),
)
...
- **[File loaders](/studio/search/search-toolkit/ingestion/loaders)**: load files from filesystem, cloud storage, or custom sources
- **[Document extractors](/studio/search/search-toolkit/ingestion/extractors)**: extract content from PDFs, HTML, spreadsheets, and more
- **[Text splitters](/studio/search/search-toolkit/ingestion/splitters)**: divide documents into chunks with the size and overlap you choose
- **[Chunk enrichers](/studio/search/search-toolkit/ingestion/enrichers)**: add custom metadata for filtering and ranking
- **[Embedders](/studio/search/search-toolkit/ingestion/embedders)**: convert text to vectors for semantic search
- **[Search index](/studio/search/search-toolkit/search-index)**: store and search over indexed chunks
