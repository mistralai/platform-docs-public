...
Pour la liste complète des types d’événements workflow et activité, consultez [Concepts clés > Événements > Types d’événements](/studio/workflows/getting-started/core_concepts/events#event-types). L’API de streaming émet également des événements **`CUSTOM_TASK_*`** générés dans les activités (démarré, en cours, terminé, échec, timeout, annulé). Utilisez ces événements pour publier l’avancement depuis des activités de longue durée ; consultez la section [Streaming](/studio/workflows/building-workflows/streaming) pour le détail de la publication.
