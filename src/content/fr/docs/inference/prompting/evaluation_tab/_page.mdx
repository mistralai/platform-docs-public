...
Il existe de nombreuses façons d'évaluer les résultats d'un LLM. Voici trois approches pour référence : inclure un score de confiance, introduire une étape d'évaluation ou utiliser un autre LLM pour l'évaluation.

### Inclure un score de confiance

Nous pouvons inclure un score de confiance avec le résultat généré.
...
- **Sortie JSON** : Pour faciliter les tâches en aval, le format JSON est fréquemment privilégié. Nous pouvons activer le mode JSON en définissant response_format sur `{"type": "json_object"}` et en précisant dans le prompt que « You will only respond with a JSON object with the key Summary and Confidence. » Spécifier ces clés dans l'objet JSON améliore la clarté et la cohérence.
- **Température plus élevée** : Dans cet exemple, nous augmentons la température pour encourager le modèle à être plus créatif et à générer trois résumés différents les uns des autres.

### Introduire une étape d'évaluation

Nous pouvons également ajouter une seconde étape dans le prompt pour l'évaluation. Nous demandons au LLM de générer trois résumés puis d'évaluer ces trois résumés.
...
### Utiliser un autre LLM pour l'évaluation

Dans les systèmes de production, il est courant d'utiliser un autre LLM pour l'évaluation afin que l'étape d'évaluation soit séparée de la génération des résumés.
...
- **Chaînage de LLM** : Dans cet exemple, nous chaînons deux LLM en séquence, où la sortie du premier LLM sert d'entrée au second LLM. La méthode de chaînage de LLM peut être adaptée à vos cas d'usage spécifiques. Par exemple, vous pouvez choisir d'utiliser trois LLM en chaîne, où la sortie de deux LLM alimente le troisième LLM. Bien que le chaînage de LLM offre de la flexibilité, il est important de noter qu'il peut entraîner des appels API supplémentaires et potentiellement des coûts accrus.
