...
The core of an agent relies on its tool usage capabilities, enabling it to call tools and workflows depending on the task it must accomplish.

Built into our API, we provide [built-in tools](built-in) such as `websearch`, `code_interpreter`, `image_generation`, and `document_library`. However, you can also use standard function tool calling by defining a JSON schema for your function.

You can also use our MCP orchestration to implement local function calling. Visit our [Local MCP docs](../mcp/#step-4-register-mcp-client) for further details.

For more information on function calling, we recommend visiting our [function calling docs](../../capabilities/function_calling).

<SectionTab as="h1" sectionId="creating-an-agent-with-function-calling">Creating an agent</SectionTab>

To use function calling, you can either create an agent or use the Conversations API directly without one.
Below we'll show you how to create an agent with function calling and use it.
...
Once defined, we provide a schema corresponding to the same function.
<Tabs groupId="code">
...
The output of the agent creation is the agent object, which contains the agent ID that we'll use to start a conversation.
...
The model will output either an answer or a function call. We need to detect the function call and return the result of the expected function.
