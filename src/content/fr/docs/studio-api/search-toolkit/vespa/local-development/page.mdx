...
`create_default_schema()` inclut par défaut tous les champs requis pour VespaSearchIndex. Utilisez `additional_fields` pour ajouter des champs personnalisés à votre cas d’usage. Pour un contrôle total sur l’ensemble des champs, utilisez `create_schema()` avec des champs explicites.
