...
Si l'agent a des guardrails configurés, ils sont automatiquement appliqués à toutes les conversations. Vous pouvez les remplacer en passant un champ `guardrails` directement sur les requêtes `POST /v1/conversations` avec un modèle. Voir [Guardrails personnalisés](/studio/safety-moderation#custom-guardrails) pour plus de détails.
...
Pour plus d'informations concernant les handoffs, consultez [cette section](/studio/agents/handoffs).
