...
Les limites de débit sont toujours partagées entre tous les workers et workflows d’un même déploiement. Le paramètre `key` contrôle la manière dont les activités partagent ces limites&nbsp;:
...
:::tip
`get_mistral_client` est la fabrique intégrée au SDK Workflows pour créer un client Mistral correctement configuré dans les activités. Il gère automatiquement l'authentification, la télémétrie et les métadonnées d'observabilité. Consultez [Client Mistral](/studio/workflows/building-workflows/mistral_client) pour plus de détails.
:::
