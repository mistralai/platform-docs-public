...
- **[Garde-fous personnalisés](#custom-guardrails)** *(recommandé)* : déclarez des règles de modération directement dans vos requêtes API — aucun appel séparé, aucune logique de seuil dans votre code. Compatible avec les chat completions, les conversations et la configuration au niveau de l'agent.
...
<Faq>
  <FaqItem question="Quelle est la distribution des résultats faux positifs et faux négatifs sur les nouveaux modèles de l'API de modération. Plus précisément, seront-ils plus susceptibles de marquer quelque chose comme nuisible alors qu'il ne l'est pas ou de ne pas marquer quelque chose qui est nuisible ?">
    Sur notre jeu de tests interne, les politiques ont une précision comprise entre [0,8 - 1,0] et un rappel entre [0,7 - 1,0]. Nous vous recommandons d'utiliser les scores bruts plutôt que les réponses booléennes si vous avez des objectifs d'application spécifiques (par exemple, réduire les faux positifs) et de définir les seuils en conséquence. Nous recueillons continuellement des retours sur les performances et améliorons nos modèles.
  </FaqItem>
</Faq>

<SectionTab as="h1" sectionId="custom-guardrails">Garde-fous personnalisés</SectionTab>

Les garde-fous personnalisés vous permettent de déclarer des règles de modération directement dans vos requêtes API, sans appeler manuellement l'API de modération et implémenter la logique de seuil dans le code de votre application. Les garde-fous appliquent **uniquement la modération des entrées** — ils s'exécutent avant que la requête n'atteigne le modèle. Lorsqu'un garde-fou est déclenché, la requête est bloquée et une erreur `403` est renvoyée.
...
- **`custom_category_thresholds`** : objet mappant les noms de catégorie aux valeurs de seuil (0–1). Définissez une catégorie à `1` pour la désactiver explicitement.
...
Plusieurs garde-fous peuvent être spécifiés par requête — la requête est bloquée si l'un d'eux est déclenché. Une seule configuration `moderation_llm_v2` peut être spécifiée par objet garde-fou, mais vous pouvez inclure plusieurs objets garde-fou.

<SectionTab as="h2" variant="secondary" sectionId="inline-guardrails">Garde-fous en ligne</SectionTab>
...
<SectionTab as="h2" variant="secondary" sectionId="conversations-guardrails">Point de terminaison Conversations</SectionTab>
