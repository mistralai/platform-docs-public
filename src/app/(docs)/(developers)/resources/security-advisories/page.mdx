...
The compromised npm packages are **inoffensive**. `Setup.mjs` references `tanstack_runner.js` but the payload file is named `router_init.js` making it useless. We still recommend removing them if you are impacted, see [IOC](#indicators-of-compromise).
...
3. Check cloud audit logs for suspicious activities.
4. Monitor connections to the following C2 indicators:
    - `api[.]masscan[.]cloud`
...
- 12/05/2026 - 15\:39 CET\: update the severity of NPM following our internal findings
