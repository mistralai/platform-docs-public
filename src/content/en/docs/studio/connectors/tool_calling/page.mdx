...
For scenarios where the model picks which tools to call, use [Connectors in conversations](/studio/connectors/conversations) instead.

:::note
If a Connector requires authentication, you must complete the [auth flow](/studio/connectors/management#get-auth-url) before calling its tools.
:::
...
:::info
Tool names must match exactly: use [list tools](/studio/connectors/management#list-tools) to check what a Connector exposes.
:::
