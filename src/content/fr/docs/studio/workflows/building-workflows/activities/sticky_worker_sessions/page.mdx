...
Le paramètre `sticky_to_worker` ne s’applique pas aux [activités locales](/studio/workflows/building-workflows/activities/local_activities), qui s’exécutent déjà dans le processus worker du workflow.

Pour une comparaison des différents types d’activités (régulières, persistantes ou locales), consultez [Choisir un type d’activité](/studio/workflows/building-workflows/activities#choosing-an-activity-flavor) sur la page parente Activités.
