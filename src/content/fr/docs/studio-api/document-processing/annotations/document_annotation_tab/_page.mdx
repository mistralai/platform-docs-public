...
Commencez par définir les formats de réponse pour l'`Annotation de documents`, en utilisant soit des schémas Pydantic ou Zod pour nos SDK, soit un schéma JSON pour un appel d'API curl.

Les schémas Pydantic/Zod/JSON acceptent les objets imbriqués, les tableaux, les énumérations, etc.
...
# Format de réponse pour l'annotation de documents
...
Vous pouvez également fournir une description pour chaque entrée ; la description sera utilisée comme information détaillée et instructions lors de l'annotation. Par exemple :
...
<SectionTab as="h3" variant="secondary" sectionId="document-annotation-prompt">Prompt d'annotation (optionnel)</SectionTab>

Après avoir défini votre schéma d'annotation, vous pouvez avoir besoin de fournir davantage de contexte et d'instructions concernant l'annotation et le document en question. Nous autorisons le passage d'un `document_annotation_prompt` qui sera utilisé comme prompt système de haut niveau pour l'étape d'annotation, fournissant un contexte et des instructions supplémentaires sur la manière dont l'annotation doit être effectuée. Un exemple de prompt serait :
...
En production, vous pouvez vouloir fournir un contexte sur le cas d'usage et affiner les instructions si la sortie ne correspond pas exactement aux attentes.

<SectionTab as="h3" variant="secondary" sectionId="document-start-request">Lancer la requête</SectionTab>

Ensuite, effectuez une requête et assurez-vous que la réponse adhère aux structures définies en utilisant `document_annotation_format` défini sur les schémas correspondants :
...
<SectionTab as="h3" variant="secondary" sectionId="document-example-output">Exemple de sortie d'annotation de documents</SectionTab>

La fonctionnalité d'annotation de documents permet d'extraire des données et d'annoter des documents. Ci-dessous, vous trouverez un exemple de sortie d'annotation :
