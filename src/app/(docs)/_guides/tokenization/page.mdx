...
Tokenization is a fundamental step in LLMs. It's the process of breaking down text into smaller subword units, known as tokens. We recently open-sourced our tokenizer. This guide walks you through the fundamentals of tokenization, details about our open-source tokenizers, and how to use them in Python.

## What is tokenization?

Tokenization is the first step and the last step of text processing and modeling. Text needs to be represented as numbers so that the model can understand it. Tokenization breaks down text into tokens, and each token is assigned a numerical representation, or index, which can be fed into a model. In a typical LLM workflow:
...
Note that we're still iterating on the tokenizer. Things may change and this is the current state of things.

We've released three versions of our tokenizers powering different sets of models.
...
source code more efficiently than the SentencePiece tokenizer used in previous Mistral models.
...
Korean and Arabic, respectively. Compared to the Llama 3 tokenizer,
...
The tokenizer doesn't encode control tokens, which helps prevent a situation known as prompt injection. For example, the control token “[INST]” is used to denote user message:
...
You may have noticed that we have 1000 slots for control tokens. The remaining 1000 - 14 = 986 slots are empty so we can add more control tokens in the future while also ensuring our vocabulary size is 131k (2\^17). Computers like powers of 2!
...
As mentioned earlier, tokenization is a crucial first step in natural language processing (NLP) tasks. Once we have tokenized our text, we can use those tokens to create text embeddings, which are dense vector representations of the text. These embeddings can then be used for a variety of NLP tasks, such as text classification, sentiment analysis, and machine translation.

Our embedding API simplifies this process by combining the tokenization and embedding steps into one. With this API, you can easily create text embeddings for a given text, without having to separately tokenize the text and create embeddings from the tokens.

If you're interested in learning more about how to use our embedding API, check out our [embedding guide](/capabilities/embeddings/overview), which provides detailed instructions and examples.

### Token count

Our LLM API endpoints charge based on the number of tokens in the input text.
