...
- **[Guardrails personnalisés](#custom-guardrails)** *(recommandé)* : déclarez les règles de modération directement dans vos requêtes API — pas d'appels séparés, pas de logique de seuil dans votre code. Compatible avec les chat completions, les conversations et la configuration au niveau de l'agent.
...
<Faq>
  <FaqItem question="Quelle est la distribution des résultats faux positifs et faux négatifs sur les nouveaux modèles d'API de modération ? Plus précisément, seront-ils plus susceptibles de signaler quelque chose comme nuisible alors que ce n'est pas le cas, ou de ne pas signaler quelque chose qui est nuisible ?">
    Sur notre ensemble de test interne, les politiques ont une précision comprise entre [0,8 - 1,0] et un rappel entre [0,7 - 1,0]. Nous vous recommandons d'utiliser les scores bruts plutôt que les réponses booléennes si vous avez des objectifs applicatifs spécifiques (par exemple, réduire les faux positifs) et de définir les seuils en conséquence. Nous recueillons continuellement des retours sur les performances et améliorons nos modèles.
  </FaqItem>
</Faq>
