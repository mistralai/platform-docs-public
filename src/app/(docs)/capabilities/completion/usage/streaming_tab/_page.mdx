...
For streaming chat completions requests, you provide a list of messages and the model returns a **stream of chunks of the completion response**.
The **latency for the first token depends mostly on the number of input tokens** corresponding to the list of input messages. After that, you receive a **stream of tokens until the model decides to stop or the maximum number of tokens is reached**.
