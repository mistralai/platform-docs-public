...
Agents are the core actors in Mistral Vibe. They are autonomous entities capable of performing tasks, making decisions, and interacting with the environment. Agents can be customized for specific use cases and extended with Skills to enhance their capabilities.  

:::note
Mistral Vibe also has `AGENTS.md` support. This feature is currently only functional when an AGENTS.md file is in the root of the workspace.
:::
...
<SectionTab as="h2" variant="secondary" sectionId="custom-agents">Custom Agents</SectionTab>

Create custom agent profiles in `~/.vibe/agents/` by creating a `.toml` file, learn more about Mistral Vibe configuration [here](/mistral-vibe/terminal/configuration):
```toml
...
<SectionTab as="h2" variant="secondary" sectionId="subagents">Subagents - Task Delegation</SectionTab>

Mistral Vibe supports subagents for delegating tasks. Subagents run independently and can perform specialized work without user interaction.  
...
Mistral Vibe's skills system allows you to extend functionality through reusable components. Skills can add new tools, slash commands, and specialized behaviors.

Mistral Vibe follows the [Agent Skills specification](https://agentskills.io/specification) for skill format and structure.
...
<SectionTab as="h2" variant="secondary" sectionId="skill-discovery">Skill Discovery</SectionTab>

Mistral Vibe discovers skills from multiple locations:
