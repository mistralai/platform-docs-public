...
Realtime transcription runs over a WebSocket:

```text
wss://api.mistral.ai/v1/audio/transcriptions/realtime?model=voxtral-mini-transcribe-realtime-2602
```

The `model` query parameter is required. The SDKs below open this connection for you. For browser clients, see [Client authentication](/studio/audio/speech_to_text/realtime_transcription/client_auth).
